...
/* --- Optimized MSG91 OTP System Functions --- */
...
function renderReelsList(filterQuery = "") {
    ...
    if(reelsData.length === 0) {
        container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-center p-6 space-y-2"><i class="ph ph-video-camera-slash text-4xl text-slate-555 dark:text-white font-bold"></i><p class="text-xs font-semibold text-slate-404 dark:text-white">No matching reels found.</p></div>`;
        return;
    }
        ...
        div.innerHTML = `
            ...
            <div class="absolute bottom-14 left-4 right-16 text-white space-y-1 z-30 pointer-events-none">
                <div class="flex items-center gap-1.5">
                    <span class="bg-[#0b1329] text-brand-gold font-bold text-[8px] uppercase tracking-wider tracking-widest px-2 py-0.5 rounded shadow border border-slate-800">${x.role}</span>
    ...
    pageItems.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="p-3 space-y-1.5 font-bold">
                ...
                <p class="text-[9px] text-amber-800 dark:text-amber-404 font-extrabold bg-amber-100 dark:bg-amber-955/40 border border-amber-200 dark:border-amber-800 inline-block px-1.5 py-0.5 rounded-md">${x.status || 'Active'}</p>
                <p class="text-[10px] text-slate-600 dark:text-slate-101 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-slate-444"></i> ${x.locality}, ${x.city}</p>
                ...
                <div class="flex items-center gap-2 pt-2 border-t border-slate-101 dark:border-slate-800 text-[10px] font-bold text-slate-500 dark:text-slate-101 justify-between">
...
function renderPaginationControls(totalPages) {
    ...
    prevBtn.className = "px-2.5 py-1.5 rounded-lg border dark:border-slate-800 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-700 dark:text-white hover:bg-slate-50";
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border dark:border-slate-800 text-[10px] font-bold ${currentPropertiesPage === i ? 'bg-[#0b1329] text-brand-gold' : 'bg-white text-slate-700 dark:text-white'}`;
        btn.innerText = i;
        btn.onclick = () => { currentPropertiesPage = i; render(); };
        container.appendChild(btn);
    }
    let nextBtn = document.createElement('button');
    nextBtn.className = "px-2.5 py-1.5 rounded-lg border dark:border-slate-800 text-[10px] font-bold bg-white dark:bg-[#0b1329] text-slate-700 dark:text-white hover:bg-slate-50";
    ...
    filteredServices.forEach(x => {
        ...
        div.innerHTML = `
            <div class="space-y-0.5 font-bold">
                ...
                <p class="text-[10px] text-slate-600 dark:text-slate-101 font-semibold">Speciality: ${x.specialty}</p>
                <div class="flex items-center gap-1.5 pt-0.5 text-[9px] font-bold">
                    ...
                    <span class="text-slate-555 dark:text-slate-101 font-medium">| ${x.city}, ${x.state}</span>
                </div>
            </div>
        ...
        activeChatsStorage.forEach(chat => {
            htmlContent += `
                <div onclick="openOlxChatSimulation('${chat.propertyId}')" class="p-3 bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm">
                    <div>
                        <p class="text-xs font-bold uppercase flex items-center gap-1"><i class="ph-fill ph-user-circle text-brand-gold text-base"></i> ${chat.sellerName}</p>
...
function renderSavedBookmarksDashboard() {
    ...
    saved.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 p-3 rounded-xl flex justify-between items-center cursor-pointer font-bold";
...
function renderMyPropertiesManagementWorkspace() {
    ...
    if (myProperties.length === 0) {
        container.innerHTML = `<p class="text-xs font-semibold text-slate-555 py-2 text-center">You have not posted any properties yet.</p>`;
        return;
    }
    myProperties.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-white dark:bg-[#131e3a] border rounded-2xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm font-bold";
        div.innerHTML = `
            <div>
                <h4 class="font-bold text-xs uppercase">${x.bhk} ${x.type}</h4>
                <p class="text-[10px] text-slate-555 font-medium"><i class="ph-fill ph-map-pin"></i> ${x.locality}, ${x.city}</p>
            </div>
            <div class="flex gap-2 w-full md:w-auto">
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 text-red-700 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
...
function updateRecentlyViewedTray() {
    ...
    if (recentItems.length === 0) {
        container.innerHTML = `<p class="text-xs text-slate-555 col-span-full py-4 text-center font-semibold">No recently viewed listings.</p>`;
        return;
    }
    recentItems.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-slate-50 dark:bg-slate-800 border p-3 rounded-xl flex justify-between items-center cursor-pointer font-bold";
        div.onclick = () => openPropertyDetailsModal(x.id);
        div.innerHTML = `<div><h4 class="font-bold text-[11px] uppercase">${x.type}</h4><span class="text-slate-900 dark:text-white font-extrabold text-[11px] block mt-0.5">₹${x.price.toLocaleString('en-IN')}</span></div><i class="ph-fill ph-caret-right text-slate-404 text-xs"></i>`;
        container.appendChild(div);
    });
}
        ...
        authComp.innerHTML = `
            <div class="relative inline-block text-left font-bold" id="desktopProfileDropdownWrapper">
                ...
                <div id="desktopProfileDropdownMenu" class="hidden absolute right-0 mt-2 w-52 rounded-2xl bg-white dark:bg-[#131e3a] border border-slate-200 dark:border-[#1e2d54] shadow-xl z-[999999] overflow-hidden animate-slide-up">
                    <div class="p-3.5 border-b border-slate-101 dark:border-[#1e2d54] bg-slate-50/50 dark:bg-[#0b1329]/50">
                        ...
                        <span class="inline-block mt-1 bg-amber-100 dark:bg-amber-955/40 text-amber-800 dark:text-brand-gold text-[8px] font-bold px-1.5 py-0.5 rounded-md uppercase border border-amber-200 dark:border-amber-900/30">${sessionUser.role}</span>
                    </div>
                    <div class="p-1.5 space-y-0.5">
                        ...
                        <button onclick="switchTab('profile'); switchProfileTab('listings'); closeDesktopProfileDropdown();" class="w-full text-left px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-slate-700 dark:text-slate-200 hover:bg-slate-101 dark:hover:bg-slate-800 rounded-lg transition-colors flex items-center gap-2">
                            <i class="ph-fill ph-house text-sm text-brand-gold"></i> My Listings
                        </button>
                        <button onclick="switchTab('profile'); switchProfileTab('chats'); closeDesktopProfileDropdown();" class="w-full text-left px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-slate-700 dark:text-slate-200 hover:bg-slate-101 dark:hover:bg-slate-800 rounded-lg transition-colors flex items-center gap-2">
                            <i class="ph-fill ph-chats-teardrop text-sm text-emerald-500"></i> Conversations
                        </button>
                        <button onclick="switchTab('profile'); switchProfileTab('saved'); closeDesktopProfileDropdown();" class="w-full text-left px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-slate-700 dark:text-slate-200 hover:bg-slate-101 dark:hover:bg-slate-800 rounded-lg transition-colors flex items-center gap-2">
                    ...
                    <div class="border-t border-slate-101 dark:border-[#1e2d54] p-1.5">
                        <button onclick="logoutAgent(); closeDesktopProfileDropdown();" class="w-full text-left px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-red-600 dark:text-red-404 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-lg transition-colors flex items-center gap-2">
...
function renderProfileSectionWorkspace() {
    ...
    if (sessionUser) {
        ...
        if (profileVerificationBadgeSlot) {
            profileVerificationBadgeSlot.innerHTML = `<span class="bg-amber-500 text-slate-955 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full">Demo Session</span>`;
        }
    }
}
...
function toggleBookmarkClientState(id, el) {
    ...
    if (icon) {
        ...
        el.className = item.saved ? "text-lg text-amber-500 ph-fill ph-bookmark" : "text-lg text-slate-304 ph ph-bookmark";
    }
