    ...
    try {
        ...
        if (localListings) {
            ...
            if (Array.isArray(parsed)) {
                parsed.forEach(item => {
                    if (item && item.id && !database.some(existing => String(existing.id) === String(item.id))) {
    ...
    pageItems.forEach(x => {
        ...
        div.innerHTML = `
            <div class="relative h-64 sm:h-72 bg-slate-900 overflow-hidden">
                ...
                ${premiumVisualBadge}
                ${(x.video && x.video !== "") ? '<span class="absolute top-2.5 right-2.5 bg-black/70 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs z-10"><i class="ph-fill ph-play text-amber-500"></i></span>' : ''}
            ...
            htmlContent += `
                <div onclick="openOlxChatSimulation('${chat.propertyId}')" class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-202 dark:border-slate-707 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-550 dark:text-slate-300 font-semibold">Unit: ${chat.propertyTitle}</p>
                    </div>
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            <div>
                ...
                <p class="text-[10px] text-slate-505 dark:text-slate-300 font-medium"><i class="ph-fill ph-map-pin"></i> ${x.locality}, ${x.city}</p>
            </div>
