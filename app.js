...
let requirementsDatabase = [
    {
        ...
        city: "Bhopal",
        locality: "Ayodhya Bypass",
        description: "Looking for an immediate purchase. Ready to move-in 2 BHK flat near main road with lift facility.",
    ...
    plans.forEach(p => {
        ...
        if (card) {
            if (p === plan) {
                ...
                card.classList.remove('border-slate-200', 'dark:border-slate-808');
            } else {
                card.classList.remove('border-brand-gold', 'bg-amber-50/10');
                card.classList.add('border-slate-200', 'dark:border-slate-808');
            }
        }
        if (circle) {
            if (p === plan) {
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    allDistDiv.className = "p-2.5 border-b border-slate-100 dark:border-slate-808 hover:bg-slate-100 dark:hover:bg-slate-808 cursor-pointer text-slate-900 dark:text-white font-bold flex justify-between items-center simple-district-item text-[11px] bg-white dark:bg-[#131e3a]";
    ...
    if (GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(dist => {
            ...
            div.className = "p-2.5 border-b hover:bg-slate-100 dark:hover:bg-slate-808 cursor-pointer text-slate-707 dark:text-white simple-district-item text-[11px] font-semibold bg-white dark:bg-[#131e3a] font-bold";
            div.innerText = dist;
    ...
    if (authComp && activeUser) {
        authComp.innerHTML = `
            <div class="relative inline-block text-left" id="desktopProfileDropdownWrapper">
                <button onclick="toggleDesktopProfileDropdown()" class="bg-slate-900 dark:bg-slate-800 text-brand-gold border border-amber-505 px-3.5 h-9 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center gap-1 cursor-pointer">
                    <i class="ph-fill ph-user-circle"></i> <span>Profile</span>
                </button>
                <div id="desktopProfileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[#131e3a] rounded-lg shadow-lg border border-slate-200 dark:border-slate-808 py-1 z-50">
                    <button onclick="switchTab('profile'); goBackToProfileDashboard();" class="block w-full text-left px-4 py-2 text-xs font-bold text-slate-707 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-808">My Dashboard</button>
                    <button onclick="logoutAgent(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-red-600 hover:bg-slate-100 dark:hover:bg-slate-808">Logout</button>
                </div>
            </div>
        `;
    }
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-202 dark:bg-slate-800 text-slate-707 dark:text-slate-202 flex items-center justify-center border border-slate-303 dark:border-slate-700 shadow-sm transition-all icon-box";
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-707 dark:text-slate-300 uppercase tracking-wide";
            }
        } else { 
...
function applySearchPillType(type, btn) {
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-202 dark:bg-[#131e3a] text-slate-707 dark:text-slate-202 flex items-center justify-center border border-slate-303 dark:border-slate-700 shadow-sm transition-all icon-box";
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-707 dark:text-slate-300 uppercase tracking-wide";
            }
        } else { 
...
function renderReelsList(filterQuery = "") {
    ...
    if(reelsData.length === 0) {
        container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-center p-6 space-y-2"><i class="ph ph-video-camera-slash text-4xl text-slate-500 dark:text-white font-bold"></i><p class="text-xs font-semibold text-slate-505 dark:text-white">No matching reels found.</p></div>`;
        return;
    }
    reelsData.forEach((x, index) => {
        ...
        let soundIconClass = isReelsMuted ? "ph-bold ph-speaker-slash text-base text-red-404 font-bold" : "ph-bold ph-speaker-high text-base text-emerald-404 font-bold";
        div.innerHTML = `
            ...
            <div class="absolute bottom-14 left-4 right-16 text-white space-y-1 z-30 pointer-events-none">
                <div class="flex items-center gap-1.5">
                    <span class="bg-[#0b1329] text-brand-gold font-bold text-[8px] uppercase tracking-wider px-2 py-0.5 rounded shadow border border-slate-808">${x.role}</span>
                    <h4 class="font-bold text-xs uppercase truncate tracking-tight drop-shadow-md text-white">${x.bhk} ${x.type}</h4>
                </div>
                <p class="text-[10px] font-semibold text-slate-202 truncate drop-shadow-md"><i class="ph-fill ph-map-pin text-xs text-red-505 inline mr-0.5"></i>${x.locality}, ${x.city}</p>
            ...
            <div class="absolute bottom-16 right-3 flex flex-col items-center gap-3 z-40 font-bold">
                ...
                <button onclick="toggleBookmarkClientState('${x.id}', this); event.stopPropagation();" class="w-10 h-10 bg-black/50 backdrop-blur-md rounded-full flex items-center justify-center text-white shadow-xl"><i class="${x.saved ? 'ph-fill ph-bookmark text-amber-505 text-base font-bold' : 'ph ph-bookmark text-base text-white'}"></i></button>
...
function openShareToolkitChannel(id) {
    ...
    // Open Graph Tags और WhatsApp Card प्रीव्यू की अनुकूलता के लिए व्यवस्थित लिंक
    let shareUrl = `${window.location.origin}${window.location.pathname}?property=${item.id}`;
    
    // सुंदर और आकर्षक सोशल मीडिया शेयरिंग संदेश
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            if (nearMeBtn) {
                ...
                nearMeBtn.add('bg-emerald-600', 'hover:bg-emerald-700');
        ...
        () => { 
            ...
            if (nearMeMobileBtn) {
                nearMeMobileBtn.classList.remove('text-slate-404');
...
function render() {
    ...
    if (hasNewGrids) {
        ...
        if (premiumItems.length === 0) {
            featuredGrid.innerHTML = `<div class="col-span-full py-4 text-center text-[11px] font-semibold text-slate-404">No premium properties found.</div>`;
        } else {
        ...
        if (plotItems.length === 0) {
            plotsGrid.innerHTML = `<div class="col-span-full py-4 text-center text-[11px] font-semibold text-slate-404">No plots found.</div>`;
        } else {
            plotItems.forEach(x => plotsGrid.appendChild(createPropertyCardNode(x)));
        }

        // Standard recent items
        let recentStandardItems = filtered.filter(x => x.type !== "Plot");
        document.getElementById('recentCounter').innerText = `${recentStandardItems.length} Recent Listings`;
        if (recentStandardItems.length === 0) {
            recentGrid.innerHTML = `<div class="col-span-full py-12 text-center text-xs font-bold text-slate-404 dark:text-white">Zero properties found.</div>`;
...
function createPropertyCardNode(x) {
    ...
    let badgeColor = x.role === "Owner" ? "from-slate-900 to-slate-800 text-brand-gold" : x.role === "Builder" ? "from-emerald-600 to-teal-500 text-white" : "from-purple-600 to-indigo-505 text-white";
    let dynamicFirmLabelStr = x.firmName ? `<p class="text-[10px] text-slate-505 dark:text-slate-303 font-extrabold truncate mt-0.5"><i class="ph-fill ph-buildings inline text-brand-gold"></i> ${x.firmName}</p>` : `<p class="text-[10px] text-brand-gold font-extrabold mt-0.5"><i class="ph-fill ph-seal-check inline"></i> Verified Owner</p>`;
    ...
    div.innerHTML = `
        <div class="relative h-64 sm:h-72 bg-slate-900 overflow-hidden">
            ...
            <span class="absolute top-9 left-2.5 bg-amber-505 text-slate-900 font-black text-[7px] uppercase tracking-widest px-2 py-0.5 rounded shadow-sm z-10">${x.status || 'READY TO MOVE'}</span>
            ${premiumVisualBadge}
            ...
            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-900 via-slate-900/60 to-transparent p-3 pt-10">
                <span class="bg-[#050b18]/85 border border-slate-808 text-brand-gold font-black text-[8px] px-2.5 py-0.5 rounded uppercase">${x.cat}</span>
        ...
        <div class="p-3.5 space-y-2 font-bold">
            <div class="flex justify-between items-start gap-1">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <button onclick="toggleBookmarkClientState('${x.id}', this); event.stopPropagation();" class="text-lg ${x.saved ? 'text-amber-500 ph-fill font-bold' : 'text-slate-303 dark:text-slate-303 ph'} ph-bookmark"></button>
                </div>
            </div>
            <p class="text-[10px] text-slate-606 dark:text-slate-303 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-brand-gold"></i> ${x.locality || ""}, ${x.city || ""}</p>
            
            <div class="pt-2 border-t border-slate-100 dark:border-slate-808 flex justify-between items-center">
                <div>${dynamicFirmLabelStr}</div>
                <span class="text-[9px] bg-slate-100 dark:bg-slate-808 text-slate-707 dark:text-white px-2 py-0.5 rounded-md font-bold">Details →</span>
            </div>
        </div>`;
    return div;
}
...
function renderPaginationControls(totalPages) {
    ...
    prevBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-slate-808 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-707 dark:text-white hover:bg-slate-50 cursor-pointer";
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-slate-808 text-[10px] font-bold cursor-pointer ${currentPropertiesPage === i ? 'bg-[#0b1329] text-brand-gold' : 'bg-white text-slate-707 dark:text-white'}`;
        btn.onclick = () => { currentPropertiesPage = i; render(); };
        btn.innerText = i;
        container.appendChild(btn);
    }
    let nextBtn = document.createElement('button');
    nextBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-[#0b1329] text-[10px] font-bold bg-white dark:bg-[#0b1329] text-slate-505 dark:text-white hover:bg-slate-50 cursor-pointer";
        ...
        div.innerHTML = `
            <div class="space-y-0.5 font-bold">
                ...
                <div class="flex items-center gap-1.5 pt-0.5 text-[9px] font-bold">
                    <span class="text-amber-505 font-bold"><i class="ph-fill ph-star"></i> ${x.rating}</span>
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        ...
        activeChatsStorage.forEach(chat => {
            ...
            htmlContent += `
                <div onclick="openOlxChatSimulationFromList('${chat.propertyId}', '${chat.buyerPhone}', '${displayName}')" class="p-3 bg-slate-55 dark:bg-slate-800/40 border border-slate-202 dark:border-slate-707 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm hover:border-brand-gold">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-505 dark:text-slate-303 font-semibold">Unit: ${chat.propertyTitle || 'Property'}</p>
                    </div>
...
function renderMyPropertiesManagementWorkspace() {
    ...
    if (myProperties.length === 0) {
        container.innerHTML = `<p class="text-xs font-semibold text-slate-505 py-2 text-center">You have not posted any properties yet.</p>`;
        return;
    }
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <button onclick="openEditPropertyModal('${x.id}')" class="bg-amber-100 hover:bg-amber-200 text-amber-850 text-[10px] font-extrabold uppercase px-3.5 py-1.5 rounded-lg border border-amber-200 w-full md:w-auto">Edit</button>
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-55 hover:bg-red-100 text-red-700 dark:bg-red-955/20 dark:text-red-404 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-100 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
...
function moveStep(st) {
    ...
    stages.forEach(s => {
        ...
        if (ind) {
            ind.className = parseInt(s) <= st ? "w-3.5 h-1 bg-[#0b1329] dark:bg-white rounded-full transition-all" : "w-3.5 h-1 bg-slate-202 dark:bg-slate-800 rounded-full transition-all font-bold";
        }
    });
...
function renderEditPhotoPreviews() {
    ...
    editSelectedPhotosBase64.forEach((base64, index) => {
        let card = document.createElement('div');
        card.className = "relative group w-14 h-14 rounded-lg overflow-hidden border border-slate-202 dark:border-slate-700 bg-slate-100 font-bold";
        card.innerHTML = `
            <img src="${base64}" class="w-full h-full object-cover">
...
function switchBrowseView(view) {
    ...
    if (view === 'properties') {
        ...
        reqBtn.className = "px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all text-slate-600 dark:text-slate-303 hover:text-slate-900 dark:hover:text-white";
    } else {
        ...
        propBtn.className = "px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all text-slate-600 dark:text-slate-303 hover:text-slate-900 dark:hover:text-white";
...
function renderBuyerRequirements() {
    ...
    requirementsDatabase.forEach(x => {
        ...
        let typeBadge = `<span class="bg-[#050b18]/70 border border-slate-808 text-brand-gold font-bold text-[9px] px-2 py-0.5 rounded uppercase">${x.bhk} ${x.type}</span>`;
            ...
            <div class="space-y-3 font-bold">
                <div class="flex justify-between items-start gap-1">
                    ...
                    <div class="text-right">
                        <p class="text-xs text-slate-500 dark:text-slate-404 uppercase">Max Budget</p>
                ...
                <div class="pt-2 border-t border-slate-100 dark:border-slate-808">
                    <p class="text-[9px] text-slate-404 dark:text-slate-404 uppercase mb-1">Requirement Details</p>
                    <p class="text-[11px] text-slate-600 dark:text-slate-202 leading-relaxed font-semibold">${x.description}</p>
                </div>
            </div>

            <div class="pt-3 border-t border-slate-100 dark:border-slate-808 flex justify-between items-center text-[10px] font-bold">
                <div>
                ...
                <div class="flex gap-2">
                    <a href="tel:${x.phone}" class="bg-slate-900 dark:bg-slate-800 hover:bg-slate-850 text-white font-bold text-[10px] uppercase px-4 py-2.5 rounded-lg flex items-center gap-1.5 transition-colors border dark:border-slate-707 shadow-sm"><i class="ph-fill ph-phone text-brand-gold text-sm"></i> Dial</a>
                    <a href="https://wa.me/91${x.phone}" target="_blank" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] uppercase px-4 py-2.5 rounded-lg flex items-center gap-1.5 transition-colors shadow-sm"><i class="ph-fill ph-whatsapp-logo text-sm"></i> WhatsApp</a>
                </div>
            </div>`;
...
async function updatePropertyFinal() {
    ...
    // Assign edited media contents to object
...
// Global messaging configurations
var configuration = {
  widgetId: "3666786c7531323437353833",
  tokenAuth: "524636THtinJCeD86a3c1bfbP1", 
  exposeMethods: true,
  success: (data) => {
      console.log('MSG91 OTP Verification Success:', data);
      if (typeof handleOtpVerificationSuccess === 'function') {
          handleOtpVerificationSuccess();
      }
  },
  failure: (error) => {
      console.log('MSG91 OTP Verification Failure:', error);
      alert("❌ OTP Verification failed. Please re-enter the correct code.");
  }
};
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-amber-100 text-amber-800 hover:bg-amber-200 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    } else {
        saveIcon.className = "ph ph-bookmark text-slate-707 text-sm";
        saveText.innerText = "Save";
        saveBtn.className = "bg-slate-100 text-slate-707 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    }
}
...
// ==========================================
// WHATSAPP CLOUD API CARD SENDING LOGIC
// ==========================================

/**
 * यह फ़ंक्शन बैकएंड API (/api/send-whatsapp-share) को कॉल करके 
 * निर्दिष्ट नंबर पर चेक-नाउ बटन वाला ऑफिशियल टेम्पलेट कार्ड भेजेगा।
 */
async function sendOfficialWhatsAppTemplateCard(propertyId, recipientPhone, recipientName = "Client") {
    let item = database.find(x => String(x.id) === String(propertyId));
    if (!item) {
        alert("⚠️ प्रॉपर्टी नहीं मिली!");
        return;
    }
    
    // फोन नंबर को साफ करें (केवल नंबर्स रखें)
    let cleanPhone = String(recipientPhone).trim().replace(/\D/g, '');
    if (cleanPhone.length > 10) {
        cleanPhone = cleanPhone.slice(-10); // आखिरी के 10 अंक लें
    }
    ...
    // बैकएंड को भेजे जाने वाला डेटा
    const payload = {
        customerPhone: cleanPhone,
        customerName: recipientName || "Client",
        bhkType: item.bhk && item.bhk !== "Any BHK Configuration" ? item.bhk : "Premium",
    ...
    try {
        ...
        let response = await fetch(`${BACKEND_BASE_URL}/api/send-whatsapp-share`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (response.ok && result.success) {
            alert("🎉 ऑफिशियल व्हाट्सएप कार्ड सफलतापूर्वक भेज दिया गया है!");
        } else {
            alert("⚠️ त्रुटि: " + (result.error || "मैसेज भेजने में विफलता। कृपया सुनिश्चित करें कि आपका मेटा टेम्पलेट अप्रूव्ड है और क्रेडेंशियल्स सही हैं।"));
        }
    } catch (err) {
        console.error("WhatsApp sending API failed:", err);
        alert("⚠️ बैकएंड सर्वर से कनेक्ट नहीं हो सका। कृपया सुनिश्चित करें कि आपका Render बैकएंड चल रहा है और उसमें /api/send-whatsapp-share एंडपॉइंट कॉन्फिगर किया गया है।");
    }
}

/**
 * इस फ़ंक्शन को कॉल करने पर स्क्रीन पर एक इनपुट पॉपअप (Prompt) खुलेगा,
 * जहाँ यूजर प्राप्तकर्ता का नाम और नंबर डालकर डायरेक्ट कार्ड भेज सकता है।
 */
function triggerPremiumWhatsAppCardFlow(id) {
    let item = database.find(x => String(x.id) === String(id));
    if (!item) return;

    let recipientPhone = prompt("📱 प्राप्तकर्ता का 10-अंकों का मोबाइल नंबर दर्ज करें:\n(इस नंबर पर ऑफिशियल 'Check Now' बटन वाला कार्ड जाएगा)");
    if (recipientPhone === null) return; // कैंसिल करने पर कुछ न करें
    
    recipientPhone = recipientPhone.trim();
    if (!recipientPhone || recipientPhone.length < 10) {
        alert("⚠️ अमान्य मोबाइल नंबर!");
        return;
    }

    let recipientName = prompt("👤 प्राप्तकर्ता का नाम दर्ज करें (वैकल्पिक):", "Client");
    if (recipientName === null) recipientName = "Client";

    sendOfficialWhatsAppTemplateCard(item.id, recipientPhone, recipientName);
}
