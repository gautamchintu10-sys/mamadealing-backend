...
function initTheme() {
    ...
    // Force light mode strictly if no theme has been selected
    if (savedTheme !== 'dark') {
        savedTheme = 'light';
...
function renderSavedBookmarksDashboard() {
    ...
    saved.forEach(x => {
        ...
        div.innerHTML = `<div><h4 class="font-bold text-[11px] text-slate-900 dark:text-white uppercase">${x.type}</h4><span class="text-slate-900 dark:text-white font-extrabold text-[11px] block mt-0.5">₹${formatIndianCurrency(x.price)}</span></div><i class="ph-fill ph-caret-right text-slate-400 text-xs"></i>`;
        grid.appendChild(div);
    });
}
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                ...
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 hover:bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-100 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
...
function updateRecentlyViewedTray() {
    ...
    recentItems.forEach(x => {
        ...
        div.className = "bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 p-3 rounded-xl flex justify-between items-center cursor-pointer font-bold";
        div.onclick = () => openPropertyDetailsModal(x.id);
        div.innerHTML = `<div><h4 class="font-bold text-[11px] uppercase text-slate-900 dark:text-white">${x.type}</h4><span class="text-slate-900 dark:text-white font-extrabold text-[11px] block mt-0.5">₹${formatIndianCurrency(x.price)}</span></div><i class="ph-fill ph-caret-right text-slate-400 text-xs"></i>`;
        container.appendChild(div);
    });
}
...
async function publishFinal() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert("⚠️ Saved locally, but failed to synchronize with backend database. Please check your network or try again.");
        }
    } catch (err) {
        console.warn("API sync fallback initiated. Render server might be asleep.");
        alert("⚠️ Saved locally, but failed to sync due to server cold start. Render server is waking up now.");
    }
}
        ...
        card.innerHTML = `
            <div class="space-y-3 font-bold">
                ...
                <div class="pt-2 border-t border-slate-200 dark:border-slate-800">
                    <p class="text-[9px] text-slate-400 dark:text-slate-400 uppercase mb-1">Requirement Details</p>
