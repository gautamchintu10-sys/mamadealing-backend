...
let database = [
    ...
    },
    {
        ...
        description: "Premium independent duplex with modern interior planning."
    }
];
...
// दोषरहित लोडिंग फ़िल्टर (Wipes/Filters out corrupted test properties to prevent JS runtime crash)
function loadLocalUserListings() {
    try {
        ...
        console.warn("Error retrieving local user listings fallback: ", e);
    }
}

function initializeApp() {
    try { initTheme(); } catch(e) { console.error(e); }
    try { loadLocalUserListings(); } catch(e) { console.error(e); }
    ...
    try { initDynamicPricingDisplays(); } catch(e) { console.error(e); }
    try { initBanners(); } catch(e) { console.error(e); }
    ...
    Object.keys(mobileTabs).forEach(k => {
        ...
        if (mEl) {
            if (k === tabId) {
                ...
                mEl.className = "flex flex-col items-center gap-0.5 cursor-pointer text-slate-505 dark:text-slate-303 w-11";
                const icon = mEl.querySelector('i');
...
function initiateRazorpayPayment(planType, price) {
    ...
    if (typeof Razorpay !== 'undefined') {
        const options = {
            ...
            "prefill": {
                "name": sessionUser.firmName || "User Node",
...
function triggerDirectPostFlow() {
    ...
    if (userPosts.length >= 10 && userSubscribedPlan === "Silver Plan") {
        alert("⚠️ You have reached the limit of 10 properties on your Silver Plan. Please upgrade to the Gold Plan.");
        openFeature('platformSubscriptionPlansModal');
        return;
    }
    if (userPosts.length >= 30 && userSubscribedPlan === "Gold Plan") {
        alert("⚠️ You have reached your limit of 30 properties on the Gold Plan.");
        return;
    }
...
function switchDetailMediaMode(mode) {
    ...
    if (mode === 'photos') {
        ...
        if (tabPhotos) tabPhotos.className = "px-2.5 py-1 rounded text-[9px] font-bold uppercase tracking-wider text-slate-900 bg-[#f59e0b] transition-all flex items-center gap-0.5";
    ...
    if (dotsTray) {
        ...
        if (total > 1) {
            ...
            for (let i = 0; i < total; i++) {
                ...
                dot.className = `w-1.5 h-1.5 rounded-full transition-all ${i === detailPhotoActiveIndex ? 'bg-slate-900 dark:bg-white scale-125' : 'bg-slate-300 dark:bg-slate-707'}`;
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    allDistDiv.className = "p-2.5 border-b border-slate-101 dark:border-slate-800 hover:bg-slate-101 dark:hover:bg-slate-808 cursor-pointer text-slate-900 dark:text-white font-bold flex justify-between items-center simple-district-item text-[11px] bg-white dark:bg-[#131e3a]";
    ...
    if (GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(dist => {
            ...
            div.className = "p-2.5 border-b hover:bg-slate-101 dark:hover:bg-slate-808 cursor-pointer text-slate-707 dark:text-white simple-district-item text-[11px] font-semibold bg-white dark:bg-[#131e3a] font-bold";
            div.innerText = dist;
...
function loadB2bCitiesForFilter() {
    ...
    if (!cService) return;
    cService.innerHTML = '<option value="All" class="dark:bg-[#131e3a] dark:text-white font-bold">Select District (All)</option>';
    if (state !== "All" && GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(ci => {
            cService.innerHTML += `<option class="dark:bg-[#131e3a] dark:text-white font-bold" value="${ci}">${ci}</option>`;
        });
    }
...
async function requestOtpViaMsg91() {
    ...
    if (sendOtpFunc && typeof sendOtpFunc === 'function') {
        try {
            ...
            console.error("MSG91 system failure fallback:", err);
...
function setupFallbackOtpFields() {
    ...
    alert("📱 Fallback: Input verification code directly to authenticate.");
}
    ...
    if (authComp && sessionUser) {
        authComp.innerHTML = `
            <div class="relative inline-block text-left" id="desktopProfileDropdownWrapper">
                ...
                <div id="desktopProfileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[#131e3a] rounded-lg shadow-lg border border-slate-202 dark:border-slate-800 py-1 z-50">
                    <button onclick="switchTab('profile'); goBackToProfileDashboard(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-slate-707 dark:text-white hover:bg-slate-101 dark:hover:bg-slate-800">My Dashboard</button>
                    <button onclick="logoutAgent(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-red-655 hover:bg-slate-101 dark:hover:bg-slate-800">Logout</button>
                </div>
            </div>
        `;
    }
...
function applyQuickBudgetFilter(min, max, btnElement) {
    ...
    if (btnElement) btnElement.classList.add('bg-slate-101', 'text-white');
}
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-555 text-slate-955 flex items-center justify-center border-2 border-amber-500 scale-110 shadow-md icon-box transition-all";
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-606 dark:text-slate-303 uppercase tracking-wide";
            }
        } else { 
    ...
    if (btn) {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-505 text-slate-955 flex items-center justify-center border-2 border-amber-500 scale-110 shadow-md icon-box transition-all";
            if (txt) {
                txt.className = "text-[10px] font-extrabold text-amber-505 uppercase tracking-wide";
            }
        } else { 
...
function applySearchPillType(type, btn) {
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-500/10 text-amber-500/80 dark:text-amber-404 flex items-center justify-center border border-amber-500/30 shadow-sm transition-all icon-box";
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-606 dark:text-slate-303 uppercase tracking-wide";
            }
        } else { 
...
function renderReelsList(filterQuery = "") {
    ...
    if(reelsData.length === 0) {
        container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-center p-6 space-y-2"><i class="ph ph-video-camera-slash text-4xl text-slate-505 dark:text-white font-bold"></i><p class="text-xs font-semibold text-slate-505 dark:text-white">No matching reels found.</p></div>`;
        return;
    }
    reelsData.forEach((x, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="absolute bottom-14 left-4 right-16 text-white space-y-1 z-30 pointer-events-none">
                ...
                <p class="text-[10px] font-semibold text-slate-202 truncate drop-shadow-md"><i class="ph-fill ph-map-pin text-xs text-red-500 inline mr-0.5"></i>${x.locality}, ${x.city}</p>
...
function triggerNearMeLocationEngine() {
    ...
    if (scanRadiusActive) {
        ...
        if (nearMeMobileBtn) {
            nearMeMobileBtn.classList.remove('text-emerald-505');
            nearMeMobileBtn.classList.add('text-slate-505');
        }
        render();
        return;
    }
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            if (nearMeMobileBtn) {
                nearMeMobileBtn.classList.remove('text-slate-505');
        ...
        () => { 
            ...
            if (nearMeMobileBtn) {
                nearMeMobileBtn.classList.remove('text-slate-400');
                nearMeMobileBtn.classList.add('text-emerald-505');
            }
            render();
        },
...
function renderRealtimeMessages(messages) {
    ...
    messages.forEach(m => {
        ...
        let bubbleClass = m.sender === 'user' ? "bg-slate-900 text-white rounded-2xl rounded-tr-none" : "bg-white text-slate-808 rounded-2xl rounded-tl-none border shadow-sm";
        div.innerHTML = `
...
function render() {
    ...
    if(filtered.length === 0) {
        grid.innerHTML = `<div class="col-span-full py-12 text-center text-xs font-bold text-slate-404 dark:text-white">Zero active matches found.</div>`;
    ...
    pageItems.forEach(x => {
        let div = document.createElement('div');
        div.className = "p-card relative overflow-hidden shadow-sm hover:shadow-md dark:bg-[#131e3a] transition-all duration-200 animate-slide-up cursor-pointer";
        ...
        let dynamicFirmLabelStr = x.firmName ? `<p class="text-[9px] text-slate-500 dark:text-slate-300 font-bold truncate mt-0.5"><i class="ph-fill ph-buildings inline"></i> ${x.firmName}</p>` : `<p class="text-[9px] text-brand-gold font-bold mt-0.5">🔒 Verified Landlord</p>`;
        ...
        div.innerHTML = `
            <div class="relative h-64 sm:h-72 bg-slate-900 overflow-hidden">
                ...
                <span class="absolute top-2.5 left-2.5 bg-gradient-to-tr ${badgeColor} font-extrabold text-[8px] uppercase tracking-wider px-2 py-0.5 rounded shadow shadow-sm z-10">${x.role}</span>
                ${premiumVisualBadge}
                ${(x.video && x.video !== "") ? '<span class="absolute top-2.5 right-2.5 bg-black/70 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs z-10"><i class="ph-fill ph-play text-amber-505"></i></span>' : ''}
                <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-955 via-slate-900/60 to-transparent p-3 pt-8">
                    ...
                    <p class="text-white text-sm font-extrabold tracking-tight mt-1">₹${(Number(x.price) || 0).toLocaleString('en-IN')}</p>
                </div>
            </div>
            <div class="p-3 space-y-1.5 font-bold">
                <div class="flex justify-between items-start gap-1">
                    ...
                    <div class="flex items-center gap-1.5">
                        <button onclick="openShareToolkitChannel('${x.id}'); event.stopPropagation();" class="text-slate-404 hover:text-brand-gold text-base ph-bold ph-share-network" title="Share Property"></button>
                        <button onclick="toggleBookmarkClientState('${x.id}', this); event.stopPropagation();" class="text-lg ${x.saved ? 'text-amber-500 ph-fill font-bold' : 'text-slate-305 dark:text-slate-300 ph'} ph-bookmark"></button>
                    </div>
                </div>
                <p class="text-[9px] text-amber-800 dark:text-amber-404 font-extrabold bg-amber-100 dark:bg-amber-955/40 border border-amber-202 dark:border-slate-800 inline-block px-1.5 py-0.5 rounded-md">${x.status || 'Active'}</p>
                <p class="text-[10px] text-slate-606 dark:text-slate-303 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-slate-404"></i> ${x.locality || ""}, ${x.city || ""}</p>
                ...
                <div class="flex items-center gap-2 pt-2 border-t border-slate-101 dark:border-slate-800 text-[10px] font-bold text-slate-505 dark:text-slate-300 justify-between">
...
function renderPaginationControls(totalPages) {
    ...
    prevBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-slate-800 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-707 dark:text-white hover:bg-slate-55 cursor-pointer";
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-slate-800 text-[10px] font-bold cursor-pointer ${currentPropertiesPage === i ? 'bg-[#0b1329] text-brand-gold' : 'bg-white text-slate-707 dark:text-white'}`;
        btn.innerText = i;
        btn.onclick = () => { currentPropertiesPage = i; render(); };
        container.appendChild(btn);
    }
    let nextBtn = document.createElement('button');
    nextBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-[#0b1329] text-[10px] font-bold bg-white dark:bg-[#0b1329] text-slate-705 dark:text-white hover:bg-slate-55 cursor-pointer";
...
function renderB2bServicesHub() {
    ...
    if(filteredServices.length === 0) {
        grid.innerHTML = `<div class="p-6 text-center text-slate-404 dark:text-white bg-white dark:bg-slate-900 rounded-xl border border-slate-202 dark:border-[#1e2d54] col-span-full font-semibold text-xs">No active partners registered in this area.</div>`;
        return;
    }
    filteredServices.forEach(x => {
        ...
        div.innerHTML = `
            <div class="space-y-0.5 font-bold">
                ...
                <p class="text-[10px] text-slate-606 dark:text-slate-303 font-semibold">Speciality: ${x.specialty}</p>
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        htmlContent = `<p class="text-xs text-slate-505 py-4 text-center font-semibold">No active conversations found.</p>`;
    } else {
        activeChatsStorage.forEach(chat => {
            htmlContent += `
                <div onclick="openOlxChatSimulation('${chat.propertyId}')" class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-202 dark:border-slate-707 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-550 dark:text-slate-303 font-semibold">Unit: ${chat.propertyTitle}</p>
                    </div>
...
function renderSavedBookmarksDashboard() {
    ...
    grid.innerHTML = "";
    if(saved.length === 0) {
        grid.innerHTML = `<p class="text-xs font-semibold text-slate-505 dark:text-white col-span-full py-4 text-center">No properties saved yet.</p>`;
        return;
    }
    saved.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-slate-55 dark:bg-slate-800 border border-slate-202 dark:border-slate-808 p-3 rounded-xl flex justify-between items-center cursor-pointer font-bold";
...
function renderMyPropertiesManagementWorkspace() {
    ...
    if (myProperties.length === 0) {
        container.innerHTML = `<p class="text-xs font-semibold text-slate-550 py-2 text-center">You have not posted any properties yet.</p>`;
        return;
    }
    myProperties.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-white dark:bg-[#131e3a] border rounded-2xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm font-bold border-slate-202 dark:border-slate-808";
        div.innerHTML = `
            <div>
                <h4 class="font-bold text-xs uppercase text-slate-900 dark:text-white">${x.bhk} ${x.type}</h4>
                <p class="text-[10px] text-slate-505 dark:text-slate-303 font-medium"><i class="ph-fill ph-map-pin"></i> ${x.locality}, ${x.city}</p>
            </div>
            <div class="flex gap-2 w-full md:w-auto">
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 text-red-750 dark:bg-red-955/20 dark:text-red-404 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-100 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
...
function updateRecentlyViewedTray() {
    ...
    if (recentItems.length === 0) {
        container.innerHTML = `<p class="text-xs text-slate-505 col-span-full py-4 text-center font-semibold">No recently viewed listings.</p>`;
        return;
    }
...
function renderPhotoPreviews() {
    ...
    selectedPhotosBase64.forEach((base64, index) => {
        ...
        card.className = "relative group w-14 h-14 rounded-lg overflow-hidden border border-slate-202 dark:border-slate-707 bg-slate-101 font-bold";
        card.innerHTML = `
            <img src="${base64}" class="w-full h-full object-cover">
            <button type="button" onclick="removeSelectedPhoto(${index})" class="absolute top-0.5 right-0.5 bg-red-650 text-white w-4.5 h-4.5 rounded-full flex items-center justify-center text-[8px] font-bold"><i class="ph-bold ph-trash font-bold"></i></button>`;
...
async function publishFinal() {
    ...
    // सुरक्षित इनपुट एक्स्ट्रैक्शन (Safe extraction checks)
    let statusInp = document.getElementById('formStatusInp')?.value || "Active";
    ...
    // Custom Floor selector checks (कस्टम टाइप किया फ़्लोर नंबर प्राप्त करें)
    let floorInp = document.getElementById('formFloorInp')?.value || "Ground Floor";
    if (floorInp === "Custom") {
        floorInp = document.getElementById('formCustomFloorInp')?.value.trim() || "Higher Floor";
    }
    ...
    let propertyPayload = {
        ...
        floorNo: floorInp, 
        age: ageInp, 
        description: descriptionVal || "Direct landlord verified listing. Clean title deed and immediate possession available."
    };
    ...
    try {
        ...
        console.warn("Local storage listing persistence error: ", e);
    }
        ...
        if (Array.isArray(liveProperties) && liveProperties.length > 0) {
            liveProperties.forEach(property => {
                ...
                let mappedDataNode = {
                    ...
                    description: property.description || "Direct landlord verified listing. Clean title deed and immediate possession available."
                };
...
function logoutAgent() {
    ...
    if (authComp) {
        authComp.innerHTML = `<button onclick="openFeature('loginModal')" class="bg-white hover:bg-slate-101 dark:bg-slate-800 text-slate-900 dark:text-white px-3.5 h-9 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center border border-slate-202 dark:border-slate-707 shadow-sm">Login</button>`;
    }
    renderProfileSectionWorkspace();
    switchTab('properties');
    updateMobileProfileTabLabel();
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-amber-100 text-amber-808 hover:bg-amber-200 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    } else {
        saveIcon.className = "ph ph-bookmark text-slate-700 text-sm";
        saveText.innerText = "Save";
        saveBtn.className = "bg-slate-101 text-slate-707 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    }
}
...
function renderProfileSectionWorkspace() {
    ...
    if (sessionUser) {
        if (pName) pName.innerText = sessionUser.firmName || "Verified User";
        if (pPhone) pPhone.innerText = `+91 ${sessionUser.phone}`;
        if (pAvatar) pAvatar.innerText = (sessionUser.firmName || "V").charAt(0).toUpperCase();
        if (pBadgeSlot) {
            pBadgeSlot.innerHTML = `<span class="bg-blue-100 text-blue-808 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider"><i class="ph-fill ph-seal-check"></i> ${sessionUser.role}</span>`;
        }
    } else {
...
function deletePropertyListing(id) {
    if (confirm("Are you sure you want to delete this listing?")) {
        ...
        try {
            ...
            console.warn("Local Storage update failure on ad deletion: ", e);
        }
