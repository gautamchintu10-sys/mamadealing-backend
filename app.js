...
// sessionUser को var में बदला गया है ताकि ग्लोबल स्कोप में कोई टकराव न हो
    ...
    Object.keys(mobileTabs).forEach(k => {
        ...
        if (mEl) {
            if (k === tabId) {
                ...
                mEl.className = "flex flex-col items-center gap-0.5 cursor-pointer text-slate-505 dark:text-slate-404 w-11";
                const icon = mEl.querySelector('i');
        ...
        authComp.innerHTML = `
            <div class="relative inline-block text-left" id="desktopProfileDropdownWrapper">
                ...
                <div id="desktopProfileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[#131e3a] rounded-lg shadow-lg border border-slate-202 dark:border-slate-800 py-1 z-50">
                    <button onclick="switchTab('profile'); goBackToProfileDashboard();" class="block w-full text-left px-4 py-2 text-xs font-bold text-slate-707 dark:text-white hover:bg-slate-101 dark:hover:bg-slate-801">My Dashboard</button>
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-202 dark:bg-slate-800 text-slate-707 dark:text-slate-202 flex items-center justify-center border border-slate-300 dark:border-slate-707 shadow-sm transition-all icon-box";
            if (txt) {
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <button onclick="openEditPropertyModal('${x.id}')" class="bg-amber-101 hover:bg-amber-202 text-amber-808 text-[10px] font-extrabold uppercase px-3.5 py-1.5 rounded-lg border border-amber-202 w-full md:w-auto">Edit</button>
...
function updateMobileProfileTabLabel() {
    ...
    if (mText) {
        if (activeUser) {
            ...
            if (mIcon) mIcon.className = "ph-fill ph-user text-base text-slate-505 dark:text-slate-202";
        } else {
            mText.innerText = "Login";
            if (mIcon) mIcon.className = "ph-fill ph-sign-in text-base text-slate-505 dark:text-slate-202";
        }
    }
}
...
function renderProfileSectionWorkspace() {
    ...
    if (activeUser) {
        ...
        if (pBadgeSlot) {
            pBadgeSlot.innerHTML = `<span class="bg-blue-101 text-blue-808 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider"><i class="ph-fill ph-seal-check"></i> ${activeUser.role}</span>`;
        }
    } else {
