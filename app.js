...
let requirementsDatabase = [
    ...
    },
    {
        ...
        city: "Bhopal",
        locality: "Kolar Road",
        description: "Need a residential plot of 1500 to 2400 sq.ft in a secured colony with clear title registry.",
        createdAt: new Date().toISOString()
    }
];
...
function switchDetailMediaMode(mode) {
    ...
    if (mode === 'photos') {
        ...
        if (tabVideo) tabVideo.className = "px-2.5 py-1 rounded text-[9px] font-bold uppercase tracking-wider text-white hover:text-brand-gold flex items-center gap-0.5 hidden shadow";
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 flex items-center justify-center border border-slate-300 dark:border-slate-700 shadow-sm transition-all icon-box";
            if (txt) {
    ...
    if (btn) {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-500 text-slate-950 flex items-center justify-center border-2 border-amber-500 scale-110 shadow-md icon-box transition-all";
            if (txt) {
...
function applySearchPillType(type, btn) {
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-200 dark:bg-[#131e3a] text-slate-700 dark:text-slate-200 flex items-center justify-center border border-slate-300 dark:border-slate-700 shadow-sm transition-all icon-box";
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide";
            }
        } else { 
...
function createPropertyCardNode(x) {
    ...
    div.innerHTML = `
        ...
        <div class="p-3 space-y-1.5 font-bold">
            ...
            <p class="text-[10px] text-slate-600 dark:text-slate-300 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-slate-400 dark:text-brand-gold"></i> ${x.locality || ""}, ${x.city || ""}</p>
            ...
            <div class="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-[10px] font-bold text-slate-500 dark:text-slate-300 justify-between">
                ...
                <div class="flex items-center gap-1.5">
                    <span title="Chat Available" class="flex items-center justify-center w-5 h-5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 text-xs font-bold"><i class="ph-fill ph-chats-teardrop"></i></span>
                    <span class="flex items-center justify-center w-5 h-5 bg-blue-50 dark:bg-blue-955/40 text-blue-600 dark:text-blue-400 text-xs font-bold" title="Call Option Enabled"><i class="ph-fill ph-phone"></i></span>
                    <span class="flex items-center justify-center w-5 h-5 bg-green-50 dark:bg-green-955/40 text-green-600 dark:text-green-400 text-xs font-bold" title="WhatsApp Connect Available"><i class="ph-fill ph-whatsapp-logo"></i></span>
                </div>
            </div>
        </div>`;
    return div;
...
function renderPaginationControls(totalPages) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-[10px] font-bold cursor-pointer ${currentPropertiesPage === i ? 'bg-[#0b1329] text-brand-gold' : 'bg-white text-slate-707 dark:text-white'}`;
        btn.onclick = () => { currentPropertiesPage = i; render(); };
        btn.innerText = i;
        container.appendChild(btn);
    }
    let nextBtn = document.createElement('button');
    nextBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-[#0b1329] text-[10px] font-bold bg-white dark:bg-[#0b1329] text-slate-500 dark:text-white hover:bg-slate-50 cursor-pointer";
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        ...
        activeChatsStorage.forEach(chat => {
            htmlContent += `
                <div onclick="openOlxChatSimulation('${chat.propertyId}')" class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm">
                    <div>
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            <div>
                ...
                <p class="text-[9px] text-brand-gold mt-1">₹${x.price.toLocaleString('en-IN')}</p>
            </div>
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                ...
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 hover:bg-red-100 text-red-700 dark:bg-red-955/20 dark:text-red-400 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-100 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
        ...
        card.innerHTML = `
            <div class="space-y-3 font-bold">
                <div class="flex justify-between items-start gap-1">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-sm font-black text-brand-gold">₹${(Number(x.budget) || 0).toLocaleString('en-IN')}</p>
                    </div>
                </div>
            ...
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-[10px] font-bold">
                ...
                <div class="flex gap-2">
                    <a href="tel:${x.phone}" class="bg-slate-900 dark:bg-slate-800 hover:bg-slate-850 text-white px-3 py-1.5 rounded-lg border dark:border-slate-700 flex items-center gap-1"><i class="ph-fill ph-phone text-brand-gold"></i> Dial</a>
    ...
    try {
        ...
        if (Array.isArray(liveProperties) && liveProperties.length > 0) {
            liveProperties.forEach(property => {
                ...
                let mappedDataNode = {
                    id: property._id || property.id || (Date.now() + Math.random()), 
