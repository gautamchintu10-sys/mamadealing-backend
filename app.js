        ...
        try {
            ...
            if (profilesRaw) {
                ...
                if (profiles[activePhone]) {
                    ...
                    userSubscribedPlan = sessionUser.subscribedPlan || "Free";
                    finishAuthNodeSetup();
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    if (GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(dist => {
            ...
            div.className = "p-2.5 border-b border-slate-200 dark:border-slate-700 hover:bg-slate-101 dark:hover:bg-slate-800 cursor-pointer text-slate-700 dark:text-slate-200 simple-district-item text-[11px] bg-white dark:bg-[#131e3a] font-bold";
            div.innerText = dist;
...
function handleOtpVerificationSuccess() {
    ...
    // Look up if user profile already exists
    let storedProfiles = {};
    try {
...
function selectRole(role) {
    ...
    // Visually highlight chosen role option card
...
function saveNewOnboardingProfile() {
    ...
    sessionUser = {
        ...
        subscribedPlan: "Free", // New accounts default to free plan
        email: ""
    };
    window.sessionUser = sessionUser;
    userSubscribedPlan = "Free";
...
// Fixed navigateToMyDashboard route to avoid race conditions
...
function finishAuthNodeSetup() {
    ...
    if (authComp && activeUser) {
        authComp.classList.remove('hidden'); // Guarantee that component wrapper is visible
        authComp.innerHTML = `
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center border border-slate-200 dark:border-slate-700 shadow-sm transition-all icon-box";
            if (txt) {
...
// Advanced WhatsApp Share Experience
function openShareToolkitChannel(id) {
    ...
    // Highly descriptive premium layout text card format
    let textMessage = `🏡 *New Verified Property Alert* 🎉\n\n` +
                      `🔥 *${displayTypeTitle}*\n` +
                      `📍 *Location:* ${item.locality || "Bhopal"}, ${item.city || "Bhopal"}\n` +
                      `💰 *Price:* ₹${formatIndianCurrency(item.price)}\n` +
                      `📐 *Area:* ${item.area ? item.area + ' Sq.Ft.' : 'N/A'}\n` +
                      `✨ *Furnishing:* ${item.furnishing || 'N/A'}\n` +
                      `🛠️ *Status:* ${item.status || 'Active'}\n\n` +
                      `👉 *Zero Brokerage - Direct Landlord Connection!*\n` +
                      `Click below to view high-quality photos, layouts & contact owner:\n` +
                      `🔗 *Explore Now:* ${shareUrl}\n\n` +
                      `_Shared via Mama Dealing Bhopal_`;
    ...
    if (sc) {
        sc.onclick = () => {
            navigator.clipboard.writeText(textMessage).then(() => {
                alert("📋 Copy successful! Premium share card details copied to your clipboard.");
...
function triggerScamReportAction() {
    ...
    if (count >= 3) {
        const targetId = targetDetailItemInstance.id;
        database = database.filter(x => String(x.id) !== String(targetId));

        // Clean user listings cache
        try {
            let raw = safeStorage.getItem('mamadealing_user_listings');
            if (raw) {
                let parsed = JSON.parse(raw);
                if (Array.isArray(parsed)) {
                    parsed = parsed.filter(x => String(x.id) !== String(targetId));
                    safeStorage.setItem('mamadealing_user_listings', JSON.stringify(parsed));
                }
            }
        } catch(e){}

        // Clean global live properties cache
        try {
            let cached = safeStorage.getItem('cached_live_properties');
            if (cached) {
                let parsed = JSON.parse(cached);
                if (Array.isArray(parsed)) {
                    parsed = parsed.filter(x => String(x.id) !== String(targetId));
                    safeStorage.setItem('cached_live_properties', JSON.stringify(parsed));
                }
            }
        } catch(e){}
        ...
        database.forEach(item => {
            ...
            const popupMiniHtml = `
                <div class="p-2 space-y-1.5 text-slate-900 font-bold" style="width: 150px; font-family: sans-serif;">
                    ...
                    <h4 class="text-[10px] uppercase truncate font-bold text-slate-900 dark:text-white">${item.bhk} ${item.type}</h4>
    ...
    if (invContainer) {
        ...
        if (matchingPartnerProperties.length === 0) {
            ...
            matchingPartnerProperties.forEach(prop => {
                ...
                itemDiv.innerHTML = `
                    <span class="text-[10px] uppercase truncate font-bold text-slate-800 dark:text-slate-100 max-w-[120px]">${prop.bhk} ${prop.type}</span>
...
function selectPremiumPlan(planType, price) {
    ...
    // Save plan directly inside individual user's profile state
        ...
        card.innerHTML = `
            <div class="space-y-3 font-bold">
                <div class="flex justify-between items-start gap-1">
                    ...
                    <div class="text-right">
                        <p class="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold">Max Budget</p>
            ...
            <div class="pt-3 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-[10px] font-bold">
                ...
                <div class="flex gap-2">
                    <a href="tel:${x.phone}" class="bg-slate-900 dark:bg-slate-800 hover:bg-slate-850 text-white px-3 py-1.5 rounded-lg border dark:border-slate-700 flex items-center gap-1"><i class="ph-fill ph-phone text-brand-gold font-medium"></i> Dial</a>
                    <a href="https://wa.me/91${x.phone}?text=Hi%20${encodeURIComponent(x.name)},%20I%20saw%20your%20requirement%20on%20Mama%20Dealing..." target="_blank" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg flex items-center gap-1"><i class="ph-fill ph-whatsapp-logo text-white"></i> WhatsApp</a>
                </div>
            </div>`;
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-amber-100 dark:bg-amber-955/40 text-amber-800 dark:text-amber-400 hover:bg-amber-202 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    } else {
        saveIcon.className = "ph ph-bookmark text-slate-500 dark:text-slate-400 text-sm";
        saveText.innerText = "Save";
        saveBtn.className = "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    }
}
...
function deletePropertyListing(id) {
    if (confirm("Are you sure you want to delete this listing?")) {
        ...
        // 1. Remove from local user listings cache
        try {
            let raw = safeStorage.getItem('mamadealing_user_listings');
            if (raw) {
        ...
        // 2. Remove from cached_live_properties cache
        try {
            let cached = safeStorage.getItem('cached_live_properties');
            if (cached) {
                let parsed = JSON.parse(cached);
                if (Array.isArray(parsed)) {
                    parsed = parsed.filter(x => String(x.id) !== String(id));
                    safeStorage.setItem('cached_live_properties', JSON.stringify(parsed));
                }
            }
        } catch (e) {
            console.warn("Cached live properties delete error:", e);
        }
