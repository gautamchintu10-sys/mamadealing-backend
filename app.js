...
const GEOGRAPHY_MATRIX = {
    ...
    "Telangana": ["Adilabad", "Bhadradri Kothagudem", "Hanumakonda", "Hyderabad", "Jagtial", "Jangaon", "Jayashankar Bhupalpally", "Jogulamba Gadwal", "Kamareddy", "Khammam", "Kumuram Bheem Asifabad", "Mahabubabad", "Mahabubnagar", "Mancherial", "Medak", "Medchal-Malkajgiri", "Mulugu", "Nagarkurnool", "Nalgonda", "Narayanpet", "Nirmal", "Nizamabad", "Peddalapli", "Rajanna Sircilla", "Rangareddy", "Sangareddy", "Siddipet", "Suryapet", "Vikarabad", "Wanaparthy", "Warangal", "Yadadri Bhuvanagiri"],
    ...
    "Jammu and Kashmir": ["Anantnag", "Bandipora", "Baramulla", "Budgam", "Doda", "Ganderbal", "Jammu", "Kathua", "Kishtwar", "Kulgam", "Kupwara", "Poonch", "Pullwama", "Rajouri", "Ramban", "Reasi", "Samba", "Shopian", "Srinagar", "Udhampur"],
    "Ladakh": ["Leh", "Kargil"],
...
// 🔥 Expanded Services Database to include Movers/Packers & Materials Suppliers
let servicesDatabase = [
    ...
    { id: 4, category: "Suppliers", title: "Bhopal Cement & Iron Sariya Traders", phone: "7000123999", specialty: "Cement, Aggregate, TMT Steel Sariya & Bricks", rating: 4.8, reviews: 29, verified: true, state: "Madhya Pradesh", city: "Bhopal" },
    { id: 5, category: "Movers", title: "Super Safe Packers & Relocation Movers", phone: "9111223399", specialty: "Safe Home Shifting, Heavy Packaging & Logistics", rating: 4.9, reviews: 45, verified: true, state: "Madhya Pradesh", city: "Bhopal" }
];
...
// 🛠️ DEFINING GLOBAL OPEN & CLOSE UTILITIES TO FIX JAVASCRIPT EXCEPTION ERRORS
...
function applyQuickBudgetFilter(min, max, btnElement) {
    ...
    bBtns.forEach(b => b.classList.remove('bg-blue-600', 'text-white'));
...
// 🔥 UPDATED: Category selection handles conditional geography display parameters
function selectB2bCategoryCard(category) {
    ...
    // Reset geofilters when navigating to a new category
    ...
    if (ciEl) ciEl.innerHTML = '<option value="All">Select District</option>';
...
// 🔥 REEL OPTIONS DROPDOWN AUTOMATIC HIDE SYSTEM
function handleReelFilterChange() {
    let stateVal = document.getElementById('reelStateFilter').value;
    let cityVal = document.getElementById('reelLocationFilter').value;

    if (stateVal !== "All" && cityVal !== "All") {
        toggleReelFilters(false);
    }
    renderReelsList();
}

function toggleReelFilters(show) {
    let filtersContainer = document.getElementById('reelFiltersContainer');
    let openBtn = document.getElementById('reelFiltersOpenBtn');
...
// 🔥 FIXED RESET FILTERS SYSTEM
function resetAllFilters() {
    const searchInp = document.getElementById('searchInput');
    const typeFilter = document.getElementById('filterType');
    const stateFilter = document.getElementById('filterStateMain');
    const cityFilter = document.getElementById('filterCityMain');
    const priceFilter = document.getElementById('priceFilter');
    const nearMeRadius = document.getElementById('nearMeRadius');
    ...
    if (priceFilter) priceFilter.value = 20000000;
    if (nearMeRadius) nearMeRadius.value = "10";
    
    updatePriceFilter(20000000);
    ...
    const buyTab = document.getElementById('tabBuyAsset');
    if (buyTab) {
        buyTab.classList.add('active', 'border-blue-500', 'text-white');
        buyTab.classList.remove('border-transparent', 'text-slate-400');
    }

    document.querySelectorAll('.quick-b-btn').forEach(b => {
...
function render() {
    ...
    let filtered = database.filter(x => {
        // Exclude Expired assets from general public grid rendering
    ...
    // 🔥 SORTING LOGIC FOR PREMIUM ADS TOP RANKING
    filtered.sort((a, b) => {
        const tierWeights = {
            "Enterprise Builder": 4,
            "Pro Business": 3,
            "Owner Premium": 2,
            "Free": 1
        };
        ...
        // Sort first by package weight tier, then by sold-out status
    ...
    if(filtered.length === 0) {
        grid.innerHTML = `<div class="col-span-full py-16 text-center space-y-2">
            ...
            <p class="text-sm font-bold text-slate-400">Zero active asset matches found in database stream.</p>
        </div>`;
        return;
    }
    ...
    filtered.forEach(x => {
        ...
        div.className = "p-card animate-slide-up relative bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-200";
        ...
        // Premium Badge Ribbon logic
        let premiumVisualBadge = (x.membershipTier && x.membershipTier !== "Free") 
            ? `<span class="absolute top-3 right-3 bg-gradient-to-r from-amber-500 to-yellow-600 text-white text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded shadow z-10"><i class="ph-fill ph-crown"></i> PRO BOOST</span>` 
            : '';
...
function renderReelsList() {
    ...
    // Filter out expired reels to keep reels stream accurate
    let reelsData = database.filter(x => x.video && x.video !== "" && !x.isPhotoReel && x.status !== "Expired");
    ...
    reelsData.forEach((x, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="absolute bottom-10 right-4 flex flex-col items-center gap-4 z-40">
                <button onclick="toggleReelsMuteState(this); event.stopPropagation();" class="w-12 h-12 bg-black/60 backdrop-blur-md rounded-full flex flex-col items-center justify-center text-white active:scale-95 transition-all shadow-xl" title="Mute / Unmute">
                    <i class="${soundIconClass} reels-mute-icon"></i>
                </button>

                <button onclick="toggleBookmarkClientState('${x.id}', this); event.stopPropagation();" class="w-12 h-12 bg-black/50 backdrop-blur-md rounded-full flex flex-col items-center justify-center text-white active:scale-95 transition-all shadow-xl">
                    <i class="${x.saved ? 'ph-fill ph-bookmark text-amber-400 text-xl' : 'ph ph-bookmark text-xl'}"></i>
                </button>
                
                <button onclick="window.open('tel:${x.phone}'); event.stopPropagation();" class="w-12 h-12 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center justify-center text-xl shadow-xl active:scale-95 transition-all ">
                    <i class="ph-fill ph-phone"></i>
                </button>

                <button onclick="openShareToolkitChannel('${x.id}'); event.stopPropagation();" class="w-12 h-12 bg-purple-600 hover:bg-purple-700 text-white rounded-full flex items-center justify-center text-xl shadow-xl active:scale-95 transition-all " title="Share Property Layout">
                    <i class="ph-bold ph-share-network"></i>
                </button>

                <button onclick="openPropertyDetailsModal('${x.id}'); event.stopPropagation();" class="w-12 h-12 bg-slate-900/80 backdrop-blur-md text-white rounded-full flex items-center justify-center text-xl shadow-xl active:scale-95 transition-all">
...
function renderRegistryServicesHub() {
    let grid = document.getElementById('registryProvidersGrid');
    if (!grid) return;
    grid.innerHTML = "";
    registryDatabase.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-white p-5 rounded-2xl border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-slide-up shadow-sm";
        div.innerHTML = `
            <div class="space-y-1">
                <div class="flex items-center gap-1.5 flex-wrap">
                    <h3 class="font-black text-sm text-slate-800 uppercase tracking-tight">${x.title}</h3>
                    ${x.verified ? `<span class="bg-emerald-50 text-emerald-600 font-extrabold text-[9px] uppercase px-2.5 py-0.5 rounded border border-emerald-200 flex items-center gap-0.5"><i class="ph-fill ph-seal-check"></i> Corporate Partner</span>` : ''}
                </div>
                <p class="text-xs text-slate-500 font-bold"><span class="text-blue-600">Speciality:</span> ${x.specialty}</p>
                <div class="flex items-center gap-1 text-xs font-black text-amber-500 pt-1">
                    <i class="ph-fill ph-star"></i> <span>${x.rating}</span>
                    <span class="text-slate-400 font-medium">(${x.reviews} reviews)</span>
                </div>
            </div>
            <a href="tel:${x.phone}" class="w-full sm:w-auto bg-slate-900 text-white text-center font-black text-xs uppercase px-4 py-3 rounded-xl flex items-center justify-center gap-1.5 shadow-sm"><i class="ph-fill ph-phone"></i> Dial Connect</a>
        `;
        grid.appendChild(div);
    });
}
...
function renderB2bServicesHub() {
    ...
    filteredServices.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-white p-4 rounded-2xl border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 animate-slide-up shadow-sm";
        
        let categoryColorBadge = "";
        if (x.category === "Legal") categoryColorBadge = "bg-emerald-50 text-emerald-700 border-emerald-200";
        else if (x.category === "Interior") categoryColorBadge = "bg-amber-50 text-amber-700 border-amber-200";
        else if (x.category === "Construction") categoryColorBadge = "bg-purple-50 text-purple-700 border-purple-200";
        else if (x.category === "Suppliers") categoryColorBadge = "bg-blue-50 text-blue-700 border-blue-200";
        else if (x.category === "Movers") categoryColorBadge = "bg-rose-50 text-rose-700 border-rose-200";

        let categoryDisplayLabel = x.category === "Suppliers" ? "Material Supplier" : x.category === "Movers" ? "Packers & Movers" : x.category;

        div.innerHTML = `
            <div class="space-y-1">
                <div class="flex items-center gap-1.5 flex-wrap">
                    <h3 class="font-black text-xs text-slate-800 uppercase tracking-tight">${x.title}</h3>
                    <span class="${categoryColorBadge} border text-[8px] font-black px-2 py-0.5 rounded uppercase flex items-center gap-0.5"><i class="ph-fill ph-seal-check"></i> ${categoryDisplayLabel} Expert</span>
                </div>
...
// 🔥 ADVANCED PROPERTY MANAGEMENT WORKSPACE WITH PERFORMANCE TRACKING ANALYTICS & AUTO-EXPIRY BADGES
function renderMyPropertiesManagementWorkspace() {
    ...
    myProperties.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-white border border-slate-100 rounded-3xl p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-5 animate-slide-up shadow-sm hover:shadow-md transition-all duration-200";
        
        let soldStatusLabel = x.isSoldOut 
            ? `<span class="bg-red-50 text-red-600 border border-red-100 font-black text-[9px] uppercase px-2.5 py-1 rounded-lg">SOLD OUT</span>` 
            : `<span class="bg-emerald-50 text-emerald-600 border border-emerald-100 font-black text-[9px] uppercase px-2.5 py-1 rounded-lg">Active Live</span>`;

        // Check if listing has reached auto-expiry threshold values
        let isExpiredNode = x.status === "Expired";
        let statusExpiryBadge = isExpiredNode 
            ? `<span class="bg-red-100 text-red-700 font-black text-[9px] uppercase px-2 py-0.5 rounded">Expired (Hidden)</span>`
            : `<span class="bg-blue-100 text-blue-700 font-black text-[9px] uppercase px-2 py-0.5 rounded">Live Active</span>`;

        let lastDate = new Date(x.lastRenewedAt || Date.now());
        let daysSinceRenewal = Math.floor((Date.now() - lastDate) / (1000 * 60 * 60 * 24));
        let daysLeft = Math.max(0, 30 - daysSinceRenewal);

        let countdownTextHTML = isExpiredNode 
            ? `<p class="text-[10px] text-red-500 font-extrabold mt-1">Listing expired after 30 days. Click 'Renew Listing' below to make it active.</p>`
            : `<p class="text-[10px] text-slate-400 font-bold mt-1">Expires in: <strong class="text-slate-700">${daysLeft} Days</strong></p>`;

        // Mock Dynamic Analytics Engine Metrics
        let mockImpressions = Math.floor(Math.random() * 800) + 120;
        let mockClicks = Math.floor(mockImpressions * 0.15) + 8;
        let mockLeads = Math.floor(mockClicks * 0.3) + 2;

        div.innerHTML = `
            <div class="space-y-3 w-full md:w-auto flex-1">
                <div class="flex items-center gap-2 flex-wrap">
                    <h4 class="font-black text-sm text-slate-800 uppercase tracking-tight">${x.bhk} ${x.type}</h4>
                    ${soldStatusLabel}
                    ${statusExpiryBadge}
                </div>
                <p class="text-xs text-slate-400 font-semibold"><i class="ph-fill ph-map-pin"></i> ${x.locality}, ${x.city}</p>
                
                <!-- Performance Statistics Bar -->
                <div class="flex flex-col gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100 max-w-md">
                    <div class="flex gap-4 text-[10px] text-slate-500 font-bold">
                        <span class="flex items-center gap-1"><i class="ph-fill ph-eye text-blue-500 text-sm"></i> Reach: <strong class="text-slate-800">${mockImpressions} Views</strong></span>
                        <span class="flex items-center gap-1"><i class="ph-fill ph-cursor-click text-indigo-500 text-sm"></i> Clicks: <strong class="text-slate-800">${mockClicks}</strong></span>
                        <span class="flex items-center gap-1"><i class="ph-fill ph-phone-call text-emerald-500 text-sm"></i> Leads: <strong class="text-slate-800">${mockLeads} Dial</strong></span>
                    </div>
                    ${countdownTextHTML}
                </div>
            </div>
            
            <div class="flex flex-wrap gap-2 w-full md:w-auto shrink-0 justify-end">
                ${isExpiredNode ? `
                    <button onclick="renewExpiredListing('${x.id}')" class="flex-1 md:flex-none text-[10px] font-black uppercase px-4 py-2.5 rounded-xl text-white bg-emerald-600 hover:bg-emerald-700 shadow-sm flex items-center justify-center gap-1">
                        <i class="ph-bold ph-arrows-counter-clockwise"></i> Renew Listing
                    </button>
                ` : `
                    <button onclick="boostPropertyAd('${x.id}')" class="flex-1 md:flex-none text-[10px] font-black uppercase px-4 py-2.5 rounded-xl text-white bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-600 hover:to-yellow-700 shadow-sm flex items-center justify-center gap-1">
                        <i class="ph-fill ph-crown"></i> Boost Top Rank
                    </button>
                `}
                <button onclick="toggleSoldOutStatus('${x.id}')" class="flex-1 md:flex-none text-[10px] font-black uppercase px-4 py-2.5 rounded-xl border hover:bg-slate-50 transition-colors ${x.isSoldOut ? 'bg-red-50 text-red-600 border-red-200' : 'bg-slate-100 text-slate-700'}">
                    <i class="ph ph-hand-palm"></i> ${x.isSoldOut ? 'Mark Active' : 'Mark Sold'}
                </button>
                <button onclick="editPropertyListing('${x.id}')" class="flex-1 md:flex-none bg-blue-50 hover:bg-blue-100 text-blue-700 text-[10px] font-black uppercase px-4 py-2.5 rounded-xl border border-blue-200">
                    <i class="ph ph-note-pencil"></i> Edit
                </button>
                <button onclick="deletePropertyListing('${x.id}')" class="flex-1 md:flex-none bg-red-50 hover:bg-red-100 text-red-700 text-[10px] font-black uppercase px-4 py-2.5 rounded-xl border border-red-200">
...
// 🔥 RENEW PROPERTY LISTING ACTION CONTROLLER (Front-End API Trigger)
async function renewExpiredListing(id) {
    if (!confirm("Do you want to renew this property listing details to keep it active for 30 more days?")) return;
    try {
        let response = await fetch(`${BACKEND_BASE_URL}/api/properties/${id}/renew`, {
            method: "POST"
        });
        if (response.ok) {
            let resData = await response.json();
            alert("🎉 Verification Success! Your property details are live again for the next 30 days.");
            
            // Update local client matrix state
            let localItem = database.find(x => x.id === id);
            if (localItem) {
                localItem.status = "Active";
                localItem.lastRenewedAt = new Date().toISOString();
            }
            render();
            renderProfileSectionWorkspace();
        } else {
            alert("❌ Server rejected listing renewal request.");
        }
    } catch(err) {
...
function boostPropertyAd(id) {
    let item = database.find(x => x.id === id);
    if (!item) return;

    if (userSubscribedPlan === "Free") {
        alert("⚠️ Boost Feature is exclusive for Premium members! Upgrading your plan will automatically rank your listings to the TOP of the search feeds.");
        openFeature('platformSubscriptionPlansModal');
    } else {
        item.membershipTier = userSubscribedPlan; // Push listing weight tier
        alert("🎉 Crown Boost Success! This listing is now locked to priority top rank.");
        render();
        renderProfileSectionWorkspace();
    }
}
...
function calculateHomeLoanEligibility() {
    ...
    if(!name || !phone || isNaN(income) || isNaN(requiredAmount)) {
        alert("⚠️ Kripya dhyan se fields ko bharein.");
        return;
    }
...
function triggerDirectPostFlow() {
    try {
        ...
        let userPhone = String(sessionUser.phone).trim();
        let userPostsCount = database.filter(x => x.phone && String(x.phone).trim() === userPhone).length;
        ...
        let locInp = document.getElementById('localityInp');
        let priceInp = document.getElementById('propPrice');
        ...
        console.error("Post flow trigger error:", err);
        alert("⚠️ JavaScript Error: " + err.message);
    }
}
...
async function triggerOtpRequestSequence() {
    ...
    if (window.location.protocol === 'file:') {
        alert("ℹ️ आप इस फ़ाइल को सीधे (file://) खोल रहे हैं। Firebase SMS वेरिफिकेशन के लिए इसे सर्वर पर चलाना आवश्यक है।\n\nकमांड प्रॉम्प्ट में 'node server.js' चलाएं और 'http://localhost:5000' खोलें।\n\nमॉक मोड एक्टिवेट किया गया है। OTP कोड '123456' का उपयोग करें।");
        activateMockOtpBypassSequence();
        return;
    }

    if (!firebaseAuth) {
        console.warn("Firebase Auth not initialized, falling back to mock authentication.");
        activateMockOtpBypassSequence();
        return;
    }
    ...
    try {
        ...
        const actionBtn = document.getElementById('authPrimaryActionSubmitBtn');
        if (actionBtn) actionBtn.innerText = "Sending SMS...";

        const confirmationResult = await firebaseAuth.signInWithPhoneNumber(formattedPhone, recaptchaVerifierGlobal);
        ...
        if (actionBtn) {
            ...
            actionBtn.onclick = verifyFirebaseOtpSequence;
        }
        alert("📩 OTP has been sent successfully to your mobile number via Firebase.");
    } catch (err) {
        console.error("Firebase Phone Auth error:", err);
        alert("⚠️ Firebase Auth request failed. Fallback bypass activated.");
        activateMockOtpBypassSequence();
    }
}
...
function activateMockOtpBypassSequence() {
    ...
    alert("⚠️ SMS API Bypass Active. Type mock code '123456' to login instantly!");
}

async function verifyFirebaseOtpSequence() {
    ...
    if (!confirmationResultGlobal) {
        alert("⚠️ No active OTP verification session found. Please re-request.");
        return;
    }

    try {
        const userCredential = await confirmationResultGlobal.confirm(otp);
        ...
        alert("✅ Secure Login Successful via Firebase Authentication!");
        ...
        alert("❌ Verification failed. Please check the OTP code.");
    }
}
...
function logoutAgent() {
    ...
    let component = document.getElementById('authDynamicComponent');
    if (component) {
        component.innerHTML = `<button onclick="openFeature('loginModal')" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg font-bold text-xs uppercase tracking-wider transition-all">Login</button>`;
    }
...
async function publishFinal() {
    ...
    let propertyPayload = {
        ...
        membershipTier: userSubscribedPlan // Store live active membership package layout 
    };
...
// 🔥 AI POWERED HOME ASSISTANT CHATBOT SIMULATION ENGINE
function sendAIAssistantMessage() {
    ...
    appendAIChatMessage(userText, 'user');
    inputEl.value = "";

    // Show simulated typing status loader
    let loaderId = "loader-" + Date.now();
    let streamContainer = document.getElementById('aiChatHistoryStream');
    if (streamContainer) {
        let loaderDiv = document.createElement('div');
        loaderDiv.id = loaderId;
        loaderDiv.className = "flex justify-start animate-pulse";
        loaderDiv.innerHTML = `<div class="bg-slate-100 text-slate-700 text-xs px-3 py-2.5 rounded-2xl max-w-[80%] font-semibold"><i class="ph-bold ph-circle-notch animate-spin mr-1"></i> AI Assistant is thinking...</div>`;
        streamContainer.appendChild(loaderDiv);
    ...
    setTimeout(() => {
        ...
        let aiResponseText = generateMockAIResponse(userText);
        appendAIChatMessage(aiResponseText, 'assistant');
    }, 1200);
}
...
function appendAIChatMessage(text, role) {
    ...
    let bubbleClass = role === 'user' 
        ? "bg-blue-600 text-white text-xs px-3.5 py-2.5 rounded-2xl rounded-tr-none max-w-[80%] font-semibold" 
        : "bg-slate-100 text-slate-800 text-xs px-3.5 py-2.5 rounded-2xl rounded-tl-none max-w-[80%] font-semibold shadow-sm";

    row.innerHTML = `<div class="${bubbleClass}">${text}</div>`;
...
function generateMockAIResponse(query) {
    ...
    if (q.includes("bhopal") || q.includes("rate") || q.includes("price") || q.includes("किमत")) {
        return "📍 भोपाल के केंट रोड, अयोध्या बायपास और कोलार रोड पर प्लॉट्स की कीमतें औसतन ₹2,200 से ₹4,500 प्रति वर्ग फीट चल रही हैं। फ्लैट्स के लिए 2 BHK की औसत कीमत ₹25 लाख से ₹45 लाख के बीच है। आप हमारे होमपेज पर अपनी पसंदीदा लोकेशन सर्च कर देख सकते हैं।";
    }
    if (q.includes("loan") || q.includes("emi") || q.includes("लोन") || q.includes("किश्त")) {
        return "🏦 होम लोन की ब्याज दरें वर्तमान में लगभग 8.4% से 9.2% प्रति वर्ष के बीच हैं। कुल प्रॉपर्टी वैल्यू का अधिकतम 80% तक लोन आसानी से स्वीकृत हो जाता है। आप हमारी किसी भी प्रॉपर्टी के नीचे दिए गए 'EMI Calculator' से अपनी मासिक किश्त का तुरंत अनुमान लगा सकते हैं।";
    }
    if (q.includes("registry") || q.includes("b2b") || q.includes("रजिस्ट्री")) {
        return "📜 भोपाल में प्रॉपर्टी रजिस्ट्री के लिए कुल मूल्य का लगभग 9.5% से 10.5% तक स्टांप ड्यूटी और रजिस्ट्रेशन charge लगता है। हमारे 'Services' टैब में जाकर आप भोपाल के टॉप कानूनी सलाहकारों और नामांतरण विशेषज्ञों से सीधे संपर्क कर सकते हैं।";
    }
    if (q.includes("post") || q.includes("add") || q.includes("विज्ञापन")) {
        return "📢 बिल्कुल! आप 'Post Ad' बटन पर क्लिक करके अपनी प्रॉपर्टी का विज्ञापन बिल्कुल मुफ्त में पोस्ट कर सकते हैं। डायरेक्ट ओनर के लिए 1 विज्ञापन और ब्रोकर्स/बिल्डर्स के लिए 3 विज्ञापन मुफ्त उपलब्ध हैं।";
    }
    if (q.includes("broker") || q.includes("builder") || q.includes("ब्रोकर")) {
        return "💼 ब्रोकर्स और बिल्डर्स के लिए हमारे पास विशेष प्रीमियम प्लांस उपलब्ध हैं। 'Membership Plans' पर क्लिक करें। प्रीमियम एक्टिवेट होने के बाद आपके सभी विज्ञापनों को सर्च फीड में सबसे ऊपर प्रमोट किया जाएगा।";
    }
    return "नमस्ते! मैं आपका ममाडीलिंग एआई असिस्टेंट हूँ। भोपाल में प्रॉपर्टी की खरीद-बिक्री, होम लोन की गणना, स्टांप ड्यूटी/रजिस्ट्री या एआई असिस्टेड सेलिंग सर्विस से संबंधित कोई भी सवाल आप मुझसे पूछ सकते हैं।";
}

async function fetchLivePropertiesFromDatabase() {
    try {
        console.log("🛰 *Pinging Live MongoDB Data Center Core Sequence...");
        ...
        liveProperties.forEach(property => {
            ...
            let mappedDataNode = {
                ...
                role: property.phone === "9000000000" || !property.phone ? "Owner" : "Broker", 
