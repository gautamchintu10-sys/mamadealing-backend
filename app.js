...
let selectedModalPlanPrice = 99; // Default Silver price updated to 99
...
function getPersistedUserPlan(phone) {
    ...
    let plan = safeStorage.getItem(`mamadealing_plan_${phone}`);
    let expiresAt = safeStorage.getItem(`mamadealing_plan_expires_${phone}`);
    ...
    try {
        ...
        if (rawProfiles) {
            ...
            if (profiles[phone]) {
                if (profiles[phone].subscribedPlan) plan = profiles[phone].subscribedPlan;
                if (profiles[phone].planExpiresAt) expiresAt = profiles[phone].planExpiresAt;
            }
        }
    } catch(e) {}
    ...
    if (plan && plan !== "Free" && expiresAt) {
        if (Date.now() > Number(expiresAt)) {
            plan = "Free";
            safeStorage.setItem(`mamadealing_plan_${phone}`, "Free");
...
// NEW: Firestore se plan fetch karne ke liye (asynchronous)
async function fetchUserPlanFromFirestore(phone) {
    if (!phone || !firestoreDb) return "Free";
    ...
    try {
        ...
        if (doc.exists) {
            ...
            if (plan !== "Free" && expiresAt && Date.now() > Number(expiresAt)) {
                plan = "Free";
                expiresAt = null;
                // Update Firestore to mark as expired
                await firestoreDb.collection('users').doc(String(phone)).set({
                    subscribedPlan: "Free",
                    planExpiresAt: null
                }, { merge: true });
            }
            ...
            if (expiresAt) {
                safeStorage.setItem(`mamadealing_plan_expires_${phone}`, expiresAt);
            } else {
                safeStorage.removeItem(`mamadealing_plan_expires_${phone}`);
            }
        ...
        try {
            ...
            if (doc.exists) {
                ...
                if (dbData) {
                    ...
                    // Cache plan in localStorage for offline access
                    safeStorage.setItem(`mamadealing_plan_${phone}`, plan);
                    if (expiresAt) {
                        safeStorage.setItem(`mamadealing_plan_expires_${phone}`, expiresAt);
                    } else {
...
function selectPremiumPlan(planType, price) {
    ...
    // Calculate 30 days validity timestamp
    ...
    if (expiresAt) {
        safeStorage.setItem(`mamadealing_plan_expires_${activeUser.phone}`, expiresAt);
    } else {
        safeStorage.removeItem(`mamadealing_plan_expires_${activeUser.phone}`);
    }
    ...
    try {
        ...
        profiles[activeUser.phone].planExpiresAt = expiresAt;
...
async function handleOtpVerificationSuccess() {
    ...
    if (firestoreDb) {
        try {
            ...
            if (doc.exists) {
                userExistsInDb = true;
                const dbData = doc.data();
                if (dbData && dbData.subscribedPlan) existingPlan = dbData.subscribedPlan;
                if (dbData && dbData.role) existingRole = dbData.role;
                if (dbData && dbData.firmName) existingFirm = dbData.firmName;
            }
        } catch(e) {
    ...
    if (userExistsInDb || storedProfiles[phone]) {
        sessionUser = storedProfiles[phone] || {
            phone: phone,
            role: existingRole,
            firmName: existingFirm || (existingRole === "Owner" ? "Direct Owner" : `${existingRole} Partner`)
        };
        
        // FIX: Agar localStorage mein plan nahi hai to Firestore se fetch karo
        let localPlan = getPersistedUserPlan(phone);
        if (localPlan === "Free" && (userExistsInDb && existingPlan !== "Free")) {
            // localStorage khali hai but Firestore mein plan hai - fetch karo
            sessionUser.subscribedPlan = existingPlan;
            sessionUser.planExpiresAt = null;
            userSubscribedPlan = existingPlan;
            
            // Firestore se puri details fetch karke cache karo
            try {
                const doc = await firestoreDb.collection('users').doc(String(phone)).get();
                if (doc.exists) {
                    const dbData = doc.data();
                    sessionUser.subscribedPlan = dbData.subscribedPlan || "Free";
                    sessionUser.planExpiresAt = dbData.planExpiresAt || null;
                    userSubscribedPlan = sessionUser.subscribedPlan;
                    
                    // localStorage mein cache karo
                    safeStorage.setItem(`mamadealing_plan_${phone}`, sessionUser.subscribedPlan);
                    if (sessionUser.planExpiresAt) {
                        safeStorage.setItem(`mamadealing_plan_expires_${phone}`, sessionUser.planExpiresAt);
                    }
                }
            } catch(e) {
            ...
            sessionUser.subscribedPlan = localPlan;
            window.sessionUser = sessionUser;
            userSubscribedPlan = sessionUser.subscribedPlan;
        }
        ...
        if (step2) {
            ...
            selectRole('Owner'); // Show Name input field for Owner by default
        }
    }
}

// -------------------------------------------------------------
// SELECT ROLE WITH OWNER NAME INPUT SUPPORT
// -------------------------------------------------------------
function selectRole(role) {
    ...
    if (extraInput) {
        extraInput.classList.remove('hidden'); // Always show name box
        if (role === "Owner") {
