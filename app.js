...
function initializeApp() {
    ...
    // Always start as Guest by default (Auto-login feature commented/removed)
    sessionUser = null;
    ...
    Object.keys(mobileTabs).forEach(k => {
        ...
        if (mEl) {
            if (k === tabId) {
                ...
                if (icon) icon.className = icon.className.replace('text-slate-500', 'text-brand-gold').replace('text-slate-400', 'text-brand-gold').replace('text-white', 'text-brand-gold');
            } else {
...
function switchDetailMediaMode(mode) {
    ...
    if (mode === 'photos') {
        ...
        if (tabPhotos) tabPhotos.className = "px-2.5 py-1 rounded text-[9px] font-bold uppercase tracking-wider text-slate-905 bg-[#f59e0b] transition-all flex items-center gap-0.5";
        ...
        if (tabVideo) tabVideo.className = "px-2.5 py-1 rounded text-[9px] font-bold uppercase tracking-wider text-slate-905 bg-[#f59e0b] transition-all flex items-center gap-0.5";
...
// Lightbox modal with both Photo and Video Full Screen support
...
function updateLightboxUI() {
    ...
    // Check if the user is currently looking at the Video View container
        ...
        authComp.innerHTML = `
            <div class="relative inline-block text-left" id="desktopProfileDropdownWrapper">
                ...
                <div id="desktopProfileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[#131e3a] rounded-lg shadow-lg border border-slate-200 dark:border-slate-800 py-1 z-50">
                    <button onclick="switchTab('profile'); goBackToProfileDashboard(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-slate-700 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-800">My Dashboard</button>
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-500/10 text-amber-500/80 dark:text-amber-400 flex items-center justify-center border border-amber-500/30 shadow-sm transition-all icon-box";
            if (txt) {
...
function handleLockedPhoneAction() {
    if (!sessionUser) {
        alert("🔒 Verification required. Log in with a verified OTP to continue.");
        openFeature('loginModal');
    } else {
...
// Prompt user for listing report reason cleanly
...
function render() {
    ...
    if(filtered.length === 0) {
        grid.innerHTML = `<div class="col-span-full py-12 text-center text-xs font-bold text-slate-400 dark:text-white">Zero active matches found.</div>`;
    ...
    pageItems.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="p-3 space-y-1.5 font-bold">
                ...
                <p class="text-[9px] text-amber-800 dark:text-amber-400 font-extrabold bg-amber-100 dark:bg-amber-950/40 border border-amber-200 dark:border-slate-800 inline-block px-1.5 py-0.5 rounded-md">${x.status || 'Active'}</p>
                <p class="text-[10px] text-slate-600 dark:text-slate-300 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-slate-400"></i> ${x.locality}, ${x.city}</p>
                ...
                <div class="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-[10px] font-bold text-slate-500 dark:text-slate-300 justify-between">
                    <span class="text-[9px] text-brand-gold font-bold">🔒 Direct Connect</span>
                    <div class="flex items-center gap-1.5">
                        <span title="Chat Available" class="flex items-center justify-center w-5 h-5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-404 text-xs font-bold"><i class="ph-fill ph-chats-teardrop"></i></span>
                        <span class="flex items-center justify-center w-5 h-5 bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-404 text-xs font-bold" title="Call Option Enabled"><i class="ph-fill ph-phone"></i></span>
                        <span class="flex items-center justify-center w-5 h-5 bg-green-50 dark:bg-green-950/40 text-green-600 dark:text-green-404 text-xs font-bold" title="WhatsApp Connect Available"><i class="ph-fill ph-whatsapp-logo"></i></span>
                    </div>
                </div>
            </div>`;
...
async function publishFinal() {
    ...
    // Extract description input data from form
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    }
}
