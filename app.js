    ...
    Object.keys(mobileTabs).forEach(k => {
        ...
        if (mEl) {
            if (k === tabId) {
                ...
                mEl.className = "flex flex-col items-center gap-0.5 cursor-pointer text-slate-500 dark:text-slate-303 w-11";
                const icon = mEl.querySelector('i');
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    allDistDiv.className = "p-2.5 border-b border-slate-101 dark:border-slate-800 hover:bg-slate-101 dark:hover:bg-slate-800 cursor-pointer text-slate-900 dark:text-white font-bold flex justify-between items-center simple-district-item text-[11px] bg-white dark:bg-[#131e3a]";
...
/* --- Voice Search Integration Engine --- */
...
/* --- Optimized Circular Icons Layout Dynamic Selection Engine --- */
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-50 dark:bg-amber-955/20 flex items-center justify-center text-slate-500 dark:text-slate-303 border border-slate-200 dark:border-slate-850 shadow-sm transition-all icon-box";
            if (txt) {
                txt.className = "text-[10px] font-extrabold text-slate-500 dark:text-slate-303 uppercase tracking-wide";
            }
        } else { 
    ...
    if (btn) {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-100 dark:bg-amber-955/40 flex items-center justify-center text-brand-gold border-2 border-brand-gold shadow-md icon-box scale-110";
            if (txt) {
...
function toggleReelsMuteState() {
    ...
    icons.forEach(icon => {
        icon.className = isReelsMuted ? "ph-bold ph-speaker-slash text-base text-red-404 reels-mute-icon font-bold" : "ph-bold ph-speaker-high text-base text-emerald-404 reels-mute-icon font-bold";
    });
}
...
function renderReelsList(filterQuery = "") {
    ...
    if(reelsData.length === 0) {
        container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-center p-6 space-y-2"><i class="ph ph-video-camera-slash text-4xl text-slate-500 dark:text-white font-bold"></i><p class="text-xs font-semibold text-slate-404 dark:text-white">No matching reels found.</p></div>`;
        return;
    }
    reelsData.forEach((x, index) => {
        ...
        div.innerHTML = `
            ...
            <div class="absolute bottom-14 left-4 right-16 text-white space-y-1 z-30 pointer-events-none">
                ...
                <p class="text-[10px] font-semibold text-slate-205 truncate drop-shadow-md"><i class="ph-fill ph-map-pin text-xs text-red-555 inline mr-0.5"></i>${x.locality}, ${x.city}</p>
...
function render() {
    ...
    pageItems.forEach(x => {
        let div = document.createElement('div');
        div.className = "p-card relative overflow-hidden shadow-sm hover:shadow-md dark:bg-[#131e3a] dark:border-[#1e2d54] transition-all duration-200 animate-slide-up cursor-pointer";
        
        div.setAttribute('onclick', `openPropertyDetailsModal('${x.id}')`);

        let badgeColor = x.role === "Owner" ? "from-slate-900 to-slate-808 text-brand-gold" : x.role === "Builder" ? "from-emerald-600 to-teal-500 text-white" : "from-purple-600 to-indigo-505 text-white";
        ...
        div.innerHTML = `
            ...
            <div class="p-3 space-y-1.5 font-bold">
                <div class="flex justify-between items-start gap-1">
                    ...
                    <button onclick="toggleBookmarkClientState('${x.id}', this); event.stopPropagation();" class="text-lg ${x.saved ? 'text-amber-500 ph-fill font-bold' : 'text-slate-304 dark:text-slate-300 ph'} ph-bookmark"></button>
                </div>
                <p class="text-[9px] text-amber-808 dark:text-amber-404 font-extrabold bg-amber-101 dark:bg-amber-955/40 border border-amber-200 dark:border-amber-800 inline-block px-1.5 py-0.5 rounded-md">${x.status || 'Active'}</p>
                ...
                <div class="flex items-center gap-2 pt-2 border-t border-slate-101 dark:border-slate-800 text-[10px] font-bold text-slate-500 dark:text-slate-300 justify-between">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="flex items-center justify-center w-5 h-5 bg-blue-50 dark:bg-blue-955/40 text-blue-606 dark:text-blue-404 text-xs font-bold" title="Call Option Enabled"><i class="ph-fill ph-phone"></i></span>
...
function renderPaginationControls(totalPages) {
    ...
    prevBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-705 dark:text-white hover:bg-slate-50 cursor-pointer";
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-[10px] font-bold cursor-pointer ${currentPropertiesPage === i ? 'bg-[#0b1329] text-brand-gold' : 'bg-white text-slate-755 dark:text-white'}`;
        btn.innerText = i;
        btn.onclick = () => { currentPropertiesPage = i; render(); };
        container.appendChild(btn);
    }
    let nextBtn = document.createElement('button');
    nextBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-[#0b1329] text-[10px] font-bold bg-white dark:bg-[#0b1329] text-slate-705 dark:text-white hover:bg-slate-50 cursor-pointer";
...
function renderB2bServicesHub() {
    ...
    if(filteredServices.length === 0) {
        grid.innerHTML = `<div class="p-6 text-center text-slate-404 dark:text-white bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-[#1e2d54] col-span-full font-semibold text-xs">No active partners registered in this area.</div>`;
        return;
    }
    filteredServices.forEach(x => {
        ...
        div.innerHTML = `
            <div class="space-y-0.5 font-bold">
                ...
                <p class="text-[10px] text-slate-606 dark:text-slate-300 font-semibold">Speciality: ${x.specialty}</p>
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        htmlContent = `<p class="text-xs text-slate-500 py-4 text-center font-semibold font-semibold">No active conversations found.</p>`;
    } else {
        activeChatsStorage.forEach(chat => {
            htmlContent += `
                <div onclick="openOlxChatSimulation('${chat.propertyId}')" class="p-3 bg-slate-50 dark:bg-slate-800 border border-slate-201 dark:border-slate-700 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-500 dark:text-slate-303 font-semibold">Unit: ${chat.propertyTitle}</p>
                    </div>
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            <div>
                ...
                <p class="text-[10px] text-slate-500 dark:text-slate-303 font-medium"><i class="ph-fill ph-map-pin"></i> ${x.locality}, ${x.city}</p>
            </div>
...
function renderPhotoPreviews() {
    ...
    selectedPhotosBase64.forEach((base64, index) => {
        ...
        card.className = "relative group w-14 h-14 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-101 font-bold";
        card.innerHTML = `
...
function logoutAgent() {
    ...
    if (authComp) {
        authComp.innerHTML = `<button onclick="openFeature('loginModal')" class="bg-white hover:bg-slate-101 dark:bg-slate-800 text-slate-905 dark:text-white px-3.5 h-9 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center border border-slate-200 dark:border-slate-700 shadow-sm">Login</button>`;
    }
    renderProfileSectionWorkspace();
...
function toggleBookmarkClientState(id, el) {
    ...
    if (icon) {
        ...
        el.className = item.saved ? "text-lg text-amber-500 ph-fill ph-bookmark" : "text-lg text-slate-305 dark:text-white ph ph-bookmark";
    }
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-amber-101 text-amber-808 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    } else {
