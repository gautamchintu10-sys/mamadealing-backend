...
// Fixed: Allow local html file opening to connect directly to Live Backend Server
...
// Demo and test properties blocker
...
function resetPostPropertyForm() {
    ...
    const fields = ['localityInp', 'formAreaInp', 'formCarpetAreaInp', 'formSuperBuiltupAreaInp', 'formPlotAreaInp', 'formBuiltupAreaInp', 'propPrice', 'propDescription', 'formCustomFloorInp', 'formCustomAgeInp'];
    fields.forEach(f => {
...
function simulateSuccessfulPayment(planType, price) {
    let confirmSim = confirm(`Payment Simulation:\n\nPlan: ${planType}\nAmount: ₹${price}\n\nProceed to authorize upgrade?`);
    if (confirmSim) {
        selectPremiumPlan(planType, price);
    }
}
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            ...
            if (txt) txt.className = "text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide";
        } else { 
    ...
    if (btn) {
        ...
        if (iconBox) { 
            ...
            if (txt) txt.className = "text-[10px] font-extrabold text-brand-gold uppercase tracking-wide";
        } else { 
    ...
    if (sc) {
        sc.onclick = () => {
            navigator.clipboard.writeText(textMessage).then(() => {
                alert("📋 Copy successful! Premium share card details copied.");
...
function openOlxChatSimulation() {
    ...
    document.getElementById('chatSellerName').innerText = item.firmName || "Direct Owner";
                ...
                .onSnapshot(doc => {
                    ...
                    if (data && data.messages) {
                        ...
                        firestoreDb.collection('chats').doc(chatId).set({
                            chatId: chatId,
                            propertyId: item.id,
                            propertyTitle: `${item.type === 'Plot' ? 'Plot Land' : (item.bhk || '') + ' ' + item.type} in ${item.locality}`,
...
function sendUserChatMessageSimulation() {
    ...
    if (firestoreDb) {
        const newMessage = {
            sender: (String(targetDetailItemInstance.phone) === String(activeUser.phone)) ? 'seller' : 'user',
            text: messageText,
            time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hour12: true})
        };
...
async function publishFinal() {
    ...
    if (floorInp === "Custom") {
        floorInp = document.getElementById('formCustomFloorInp')?.value.trim() || "Custom Floor";
    }

    let ageInp = document.getElementById('formAgeInp')?.value || "New Construction";
    if (ageInp === "Custom") {
        ageInp = document.getElementById('formCustomAgeInp')?.value.trim() || "Custom Age";
    }
    ...
    const userPlan = getPersistedUserPlan(activeUser ? activeUser.phone : "");
    ...
    try {
        ...
        if (response.ok) alert("🎉 Asset successfully registered!");
        else alert("🎉 Asset saved & synchronized!");
    } catch (err) {
        alert("🎉 Asset saved locally & synchronized!");
    }
}
