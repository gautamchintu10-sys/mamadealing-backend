...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            ...
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-700 dark:text-slate-303 uppercase tracking-wide";
            }
        } else { 
    ...
    div.innerHTML = `
        ...
        <div class="p-3.5 space-y-2 font-bold">
            ...
            <div class="pt-2 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                ...
                <span class="text-[9px] bg-slate-101 dark:bg-slate-800 text-slate-700 dark:text-white px-2 py-0.5 rounded-md font-bold">Details →</span>
            </div>
        </div>`;
    return div;
}
            ...
            htmlContent += `
                <div onclick="openOlxChatSimulationFromList('${chat.propertyId}', '${chat.buyerPhone}', '${displayName}')" class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm hover:border-brand-gold">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-500 dark:text-slate-300 font-semibold">Unit: ${chat.propertyTitle || 'Property'}</p>
                    </div>
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <button onclick="openEditPropertyModal('${x.id}')" class="bg-amber-100 hover:bg-amber-200 text-amber-800 text-[10px] font-extrabold uppercase px-3.5 py-1.5 rounded-lg border border-amber-202 w-full md:w-auto">Edit</button>
...
function renderProfileSectionWorkspace() {
    ...
    if (sessionUser) {
        ...
        if (pBadgeSlot) {
            pBadgeSlot.innerHTML = `<span class="bg-blue-101 text-blue-800 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider"><i class="ph-fill ph-seal-check"></i> ${sessionUser.role}</span>`;
        }
    } else {
