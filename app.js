...
/* UPDATED & FIXED: Restore Subscription from Firestore on Login */
async function handleOtpVerificationSuccess() {
    ...
    // Check Firestore first before overriding state
    if (firestoreDb) {
        try {
            const doc = await firestoreDb.collection('users').doc(String(phone)).get();
            if (doc.exists) {
    ...
    if (userExistsInDb || storedProfiles[phone]) {
        ...
        alert(`✅ Welcome back! Active Plan: ${existingPlan}`);
        renderProfileSectionWorkspace();
        render();
    } else {
        const step1 = document.getElementById('authStep1');
...
/* UPDATED & FIXED: Preserve Existing Plan on Onboarding Completion */
async function saveNewOnboardingProfile() {
    ...
    // Check Firestore doc again to be 100% sure we don't downgrade
    if (firestoreDb) {
        try {
            const doc = await firestoreDb.collection('users').doc(String(phone)).get();
