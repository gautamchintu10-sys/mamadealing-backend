...
// Demo property identifier filter
...
// Synced Plan Fetcher across Mobile & Desktop strictly tied to Phone Number
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        b.classList.remove('bg-brand-gold', 'text-slate-900');
        b.classList.add('bg-slate-100', 'dark:bg-[#131e3a]', 'text-slate-700', 'dark:text-slate-200');
    });
    
    if (btn) {
        btn.classList.add('active');
        btn.classList.remove('bg-slate-100', 'dark:bg-[#131e3a]', 'text-slate-700', 'dark:text-slate-200');
        btn.classList.add('bg-brand-gold', 'text-slate-900');
    }
...
async function publishFinal() {
    ...
    // Handle Custom Construction Age Input
