...
// Helper: Normalize any phone number to 10 digits
...
// -------------------------------------------------------------
// SUBSCRIPTION & 30-DAY EXPIRY PERSISTENCE LOGIC
// -------------------------------------------------------------
function getPersistedUserPlan(phone) {
    let cPhone = cleanPhone(phone);
    const activeUser = window.sessionUser || sessionUser;

    if (activeUser && cleanPhone(activeUser.phone) === cPhone) {
        if (activeUser.subscribedPlan && activeUser.subscribedPlan !== "Free") {
            if (!activeUser.planExpiresAt || Date.now() <= Number(activeUser.planExpiresAt)) {
                return activeUser.subscribedPlan;
            }
        }
    }

    if (!cPhone) return safeStorage.getItem('mamadealing_active_user_plan') || "Free";

    let plan = safeStorage.getItem(`mamadealing_plan_${cPhone}`);
    let expiresAt = safeStorage.getItem(`mamadealing_plan_expires_${cPhone}`);
    ...
    try {
        ...
        if (rawProfiles) {
            ...
            if (profiles[cPhone]) {
                if (profiles[cPhone].subscribedPlan) plan = profiles[cPhone].subscribedPlan;
                if (profiles[cPhone].planExpiresAt) expiresAt = profiles[cPhone].planExpiresAt;
            }
        }
    } catch(e) {}

    // Check if 30-day plan has expired
    if (plan && plan !== "Free" && expiresAt) {
        if (Date.now() > Number(expiresAt)) {
            plan = "Free";
            safeStorage.setItem(`mamadealing_plan_${cPhone}`, "Free");
            safeStorage.removeItem(`mamadealing_plan_expires_${cPhone}`);
            safeStorage.setItem('mamadealing_active_user_plan', "Free");
            
            if (firestoreDb) {
                try {
                    firestoreDb.collection('users').doc(cPhone).set({
                        subscribedPlan: "Free",
                        planExpiresAt: null
                    }, { merge: true });
                } catch(e) {}
    ...
    return plan || safeStorage.getItem('mamadealing_active_user_plan') || "Free";
}

async function fetchUserPlanFromFirestore(phone) {
    let cPhone = cleanPhone(phone);
    if (!cPhone || !firestoreDb) return "Free";
    
    try {
        let doc = await firestoreDb.collection('users').doc(cPhone).get();
        if (!doc.exists) {
            doc = await firestoreDb.collection('users').doc("91" + cPhone).get();
        }
        
        if (doc.exists) {
            const dbData = doc.data();
            let plan = dbData.subscribedPlan || "Free";
            let expiresAt = dbData.planExpiresAt || null;
            
            if (plan !== "Free" && expiresAt && Date.now() > Number(expiresAt)) {
                plan = "Free";
                expiresAt = null;
                await firestoreDb.collection('users').doc(cPhone).set({
                    subscribedPlan: "Free",
                    planExpiresAt: null
                }, { merge: true });
            }
            
            safeStorage.setItem(`mamadealing_plan_${cPhone}`, plan);
            if (expiresAt) {
                safeStorage.setItem(`mamadealing_plan_expires_${cPhone}`, expiresAt);
            } else {
                safeStorage.removeItem(`mamadealing_plan_expires_${cPhone}`);
            }
        ...
        console.warn("Firestore plan fetch error:", e);
    }
    ...
    try {
        ...
        if (doc.exists) {
            ...
            if (dbData) {
                ...
                if (sessionUser && cleanPhone(sessionUser.phone) === cPhone) {
                    sessionUser.subscribedPlan = plan;
                    sessionUser.planExpiresAt = expiresAt;
                    if (dbData.firmName) sessionUser.firmName = dbData.firmName;
                    if (dbData.role) sessionUser.role = dbData.role;
                    if (dbData.updatedAt) sessionUser.updatedAt = dbData.updatedAt;
                }
                if (window.sessionUser && cleanPhone(window.sessionUser.phone) === cPhone) {
                    window.sessionUser.subscribedPlan = plan;
                    window.sessionUser.planExpiresAt = expiresAt;
                    if (dbData.firmName) window.sessionUser.firmName = dbData.firmName;
                    if (dbData.role) window.sessionUser.role = dbData.role;
                }
...
function triggerDirectPostFlow() {
    ...
    if (activePlan === "Free") {
        const myListings = database.filter(x => cleanPhone(x.phone) === cleanPhone(activeUser.phone) && !isDemoProperty(x));
...
function initializeApp() {
    ...
    window.addEventListener('storage', function(e) {
        if (e.key === 'silver_price' || e.key === 'gold_price') {
            initDynamicPricingDisplays();
        }
        if (e.key === 'desktop_banner_image' || e.key === 'mobile_banner_image') {
            initBanners();
        }
    });

    const activePhone = cleanPhone(safeStorage.getItem('mamadealing_active_session_phone'));
...
function initDynamicPricingDisplays() {
    const sPrice = safeStorage.getItem('silver_price') || '99';
...
function openFeature(id) { 
    ...
    if (id === 'platformSubscriptionPlansModal') {
        initDynamicPricingDisplays();
    }
}
...
function selectPlanOption(plan, price) {
    // Updated pricing: Silver ₹299, Gold ₹599
    if (plan === "Silver") {
        price = 299;
    } else if (plan === "Gold") {
        price = 599;
    ...
    const plans = ["Free", "Silver", "Gold"];
    plans.forEach(p => {
        const card = document.getElementById(`planCard-${p}`);
        const circle = document.getElementById(`planCircle-${p}`);
        if (card) {
            if (p === plan) {
    ...
    const ctaText = document.getElementById('ctaButtonText');
    if (mainBtn) {
        if (plan === "Free") {
            mainBtn.innerText = `Continue with ${plan} Plan →`;
        } else {
            mainBtn.innerText = `Subscribe for ₹${price} →`;
        }
    }
    if (ctaText) {
        if (plan === "Free") {
            ctaText.innerText = `Continue with ${plan} Plan →`;
        } else {
            ctaText.innerText = `Subscribe for ₹${price} →`;
        }
    }
}
...
function initiateRazorpayPayment(planType, price) {
    ...
    if (typeof Razorpay !== 'undefined') {
        const options = {
            ...
            "currency": "INR",
            "name": "Mama Dealing",
            "description": `${planType} Subscription (30 Days)`,
...
function selectPremiumPlan(planType, price) {
    ...
    let cPhone = cleanPhone(activeUser.phone);
    const expiresAt = (planType === "Free") ? null : (Date.now() + (30 * 24 * 60 * 60 * 1000));

    activeUser.subscribedPlan = planType;
    activeUser.planExpiresAt = expiresAt;

    if (window.sessionUser) {
        window.sessionUser.subscribedPlan = planType;
        window.sessionUser.planExpiresAt = expiresAt;
    }
    userSubscribedPlan = planType;
    
    safeStorage.setItem(`mamadealing_plan_${cPhone}`, planType);
    if (expiresAt) {
        safeStorage.setItem(`mamadealing_plan_expires_${cPhone}`, expiresAt);
    } else {
        safeStorage.removeItem(`mamadealing_plan_expires_${cPhone}`);
    ...
    try {
        ...
        if (!profiles[cPhone]) {
            profiles[cPhone] = activeUser;
        }
        profiles[cPhone].subscribedPlan = planType;
        profiles[cPhone].planExpiresAt = expiresAt;
        safeStorage.setItem('mamadealing_user_profiles', JSON.stringify(profiles));
    } catch(e){}

    if (firestoreDb && cPhone) {
        try {
            const payload = {
                phone: cPhone,
            ...
            firestoreDb.collection('users').doc(cPhone).set(payload, { merge: true });
            firestoreDb.collection('users').doc("91" + cPhone).set(payload, { merge: true });
        } catch(e) {}
    }

    alert(`🎉 Success! Upgraded to ${planType}. Valid for 30 Days.`);
    closeFeature('platformSubscriptionPlansModal');
    renderProfileSectionWorkspace();
    render();
}
...
async function requestOtpViaMsg91() {
    ...
    let cPhone = cleanPhone(phoneVal);
    if (!cPhone || cPhone.length !== 10) {
        alert("⚠️ Please enter a valid 10-digit phone number.");
        return;
    }
    currentSimulatedPhoneNumber = cPhone;
...
// -------------------------------------------------------------
// CLOUD-FIRST OTP VERIFICATION (MOBILE & DESKTOP SYNCED)
// -------------------------------------------------------------
async function handleOtpVerificationSuccess() {
    let rawPhone = currentSimulatedPhoneNumber || "7869824155";
    let phone = cleanPhone(rawPhone);
    
    let activePlan = "Free";
    let planExpiresAt = null;
    let existingRole = "Owner";
    let existingFirm = "";
    let userExistsInDb = false;

    // 1. Fetch from Firestore using Both Document Key Formats (10-digit and 91+10digit)
    if (firestoreDb) {
        try {
            let doc = await firestoreDb.collection('users').doc(phone).get();
            if (!doc.exists) {
                doc = await firestoreDb.collection('users').doc("91" + phone).get();
            }
            ...
            if (doc.exists) {
                ...
                const dbData = doc.data();
                if (dbData) {
                    let fetchedPlan = dbData.subscribedPlan || "Free";
                    let fetchedExpires = dbData.planExpiresAt || null;

                    // Check expiration
                    if (fetchedPlan !== "Free" && fetchedExpires && Date.now() > Number(fetchedExpires)) {
                        fetchedPlan = "Free";
                        fetchedExpires = null;
                        await firestoreDb.collection('users').doc(phone).set({
                            subscribedPlan: "Free",
                            planExpiresAt: null
                        }, { merge: true });
                    }
                    ...
                    planExpiresAt = fetchedExpires;
                    if (dbData.role) existingRole = dbData.role;
                    if (dbData.firmName) existingFirm = dbData.firmName;
                }
            }
        } catch(e) {
            console.warn("Firestore user lookup fallback:", e);
        }
    }

    if (!userExistsInDb) {
        activePlan = getPersistedUserPlan(phone);
    }

    // Cache updated plan in LocalStorage for offline and quick access
    safeStorage.setItem(`mamadealing_plan_${phone}`, activePlan);
    if (planExpiresAt) {
        safeStorage.setItem(`mamadealing_plan_expires_${phone}`, planExpiresAt);
    } else {
        safeStorage.removeItem(`mamadealing_plan_expires_${phone}`);
    }
    safeStorage.setItem('mamadealing_active_user_plan', activePlan);
    ...
    if (userExistsInDb || storedProfiles[phone]) {
        let existingProfile = storedProfiles[phone] || {};
        sessionUser = {
            phone: phone,
            role: existingRole || existingProfile.role || "Owner",
            firmName: existingFirm || existingProfile.firmName || (existingRole === "Owner" ? "Direct Owner" : `${existingRole} Partner`),
            subscribedPlan: activePlan,
            planExpiresAt: planExpiresAt
        };
        
        window.sessionUser = sessionUser;
        userSubscribedPlan = activePlan;
        ...
        alert(`✅ Welcome back! Active Plan: ${sessionUser.subscribedPlan}`);
        renderProfileSectionWorkspace();
        render();
    } else {
        const step1 = document.getElementById('authStep1');
...
function selectRole(role) {
    ...
    let firmInput = document.getElementById('authOnboardingFirmName');

    if (extraInput) {
        extraInput.classList.remove('hidden');
        if (role === "Owner") {
            if (labelNode) labelNode.innerText = "Your Full Name / Owner Name";
            if (firmInput) firmInput.placeholder = "e.g. Rahul Sharma";
        } else if (role === "Broker") {
            if (labelNode) labelNode.innerText = "Broker / Agency Firm Name";
            if (firmInput) firmInput.placeholder = "e.g. Verma Realty Group";
        } else if (role === "Builder") {
            if (labelNode) labelNode.innerText = "Infrastructure Builder Title";
            if (firmInput) firmInput.placeholder = "e.g. Rudra Builders";
        }
    }
}

async function saveNewOnboardingProfile() {
    let rawPhone = currentSimulatedPhoneNumber || "7869824155";
    let phone = cleanPhone(rawPhone);
    let role = authSelectedRole || "Owner";
    let enteredName = document.getElementById('authOnboardingFirmName')?.value.trim() || "";

    let existingPlan = getPersistedUserPlan(phone);
    ...
    if (firestoreDb) {
        try {
            ...
            if (doc.exists && doc.data()?.subscribedPlan) {
                existingPlan = doc.data().subscribedPlan;
            }
        } catch(e){}
    }
    ...
    sessionUser = {
        phone: phone,
        role: role,
        firmName: enteredName || (role === "Owner" ? "Direct Owner" : `${role} Partner`),
        subscribedPlan: existingPlan,
        email: ""
    };
    window.sessionUser = sessionUser;
    userSubscribedPlan = existingPlan;

    safeStorage.setItem(`mamadealing_plan_${phone}`, existingPlan);
    safeStorage.setItem('mamadealing_active_user_plan', existingPlan);
    ...
    if (firestoreDb) {
        try {
            ...
            await firestoreDb.collection('users').doc(phone).set(payload, { merge: true });
            await firestoreDb.collection('users').doc("91" + phone).set(payload, { merge: true });
        } catch(e){}
    }
...
function startListeningToMyChats() {
    try {
        ...
        const userPhoneStr = cleanPhone(activeUser.phone);
        ...
        const handleChatsUpdate = (querySnapshot) => {
            querySnapshot.forEach(doc => {
                ...
                if (data.messages && data.messages.length > 0) {
                    ...
                    const buyerPhoneVal = cleanPhone(data.buyerPhone);
                    const myPhoneVal = cleanPhone(activeUser.phone);
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        const isMobileTab = b.querySelector('.icon-circle') || b.querySelector('div');
        ...
        if (isMobileTab) {
            ...
            if (b === btn) {
                ...
                if (circle) circle.className = "icon-circle w-12 h-12 rounded-full bg-brand-gold text-slate-900 flex items-center justify-center shadow-md font-bold text-xl transition-all scale-105";
                if (label) label.className = "tab-label text-[10px] font-extrabold uppercase text-brand-gold tracking-wider";
            } else {
                b.classList.remove('active');
                if (circle) circle.className = "icon-circle w-12 h-12 rounded-full bg-slate-100 dark:bg-[#131e3a] border border-slate-200 dark:border-slate-700 text-brand-gold flex items-center justify-center shadow-sm font-bold text-xl transition-all";
                if (label) label.className = "tab-label text-[10px] font-bold uppercase text-slate-600 dark:text-slate-300 tracking-wider";
            }
        } else {
            if (b === btn) {
                b.className = "custom-search-tab active px-3.5 py-1.5 rounded-md bg-white text-slate-900 font-extrabold shadow-sm shrink-0 transition-all text-xs";
                b.style.backgroundColor = '#ffffff';
            } else {
                b.className = "custom-search-tab px-3.5 py-1.5 rounded-md text-slate-900 font-bold hover:bg-amber-600 transition-colors shrink-0 text-xs";
                b.style.backgroundColor = 'transparent';
            }
        }
    });
...
function getChatOnlineStatusText(itemPhone) {
    ...
    const activeUser = window.sessionUser || sessionUser;
    if (activeUser && cleanPhone(activeUser.phone) === cleanPhone(itemPhone)) {
        return "Online";
    }
...
function openOlxChatSimulation(id) {
    ...
    const chatId = `chat_${targetProp.id}_${cleanPhone(activeUser.phone)}`;
                ...
                .onSnapshot(doc => {
                    ...
                    if (data && data.messages) {
                        ...
                        firestoreDb.collection('chats').doc(chatId).set({
                            ...
                            buyerPhone: cleanPhone(activeUser.phone),
                            buyerName: activeUser.firmName || "Interested Buyer",
                            sellerPhone: cleanPhone(targetProp.phone),
...
function openOlxChatSimulationFromList(propertyId, buyerPhone, sellerName) {
    ...
    const chatId = `chat_${propertyId}_${cleanPhone(buyerPhone)}`;
...
function renderRealtimeMessages(messages) {
    ...
    messages.forEach(m => {
        ...
        if (m.sender === 'user') {
            isMe = (cleanPhone(targetDetailItemInstance?.phone) !== cleanPhone(activeUser?.phone));
...
function sendUserChatMessageSimulation() {
    ...
    const chatId = `chat_${targetDetailItemInstance.id}_${cleanPhone(activeUser.phone)}`;

    if (firestoreDb) {
        const newMessage = {
            sender: (cleanPhone(targetDetailItemInstance.phone) === cleanPhone(activeUser.phone)) ? 'seller' : 'user',
            text: messageText,
            time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hour12: true})
        };
...
function openPartnerProfileModal(phone) {
    let cPhone = cleanPhone(phone);
    const matchingPartnerProperties = database.filter(x => cleanPhone(x.phone) === cPhone && !isDemoProperty(x));
    ...
    document.getElementById('partnerModalPhone').innerText = `+91 ${cPhone}`;
    ...
    if (callBtn) callBtn.href = `tel:${cPhone}`;
    if (waBtn) waBtn.href = `https://wa.me/91${cPhone}?text=Hi%20there%2C%20I%20am%20interested%20in%20your%20properties%20on%20Mama%20Dealing...`;
...
function submitNewB2bDirectoryProvider() {
    ...
    let phone = cleanPhone(document.getElementById('b2bFormPhone').value.trim());
...
function submitVipCallbackRequest() {
    ...
    let phone = cleanPhone(document.getElementById('vipCallbackPhone').value.trim());
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        ...
        activeChatsStorage.forEach(chat => {
            let displayName = cleanPhone(chat.buyerPhone) === cleanPhone(activeUser.phone) 
...
function renderMyPropertiesManagementWorkspace() {
    ...
    let myProperties = database.filter(x => cleanPhone(x.phone) === cleanPhone(activeUser.phone) && !isDemoProperty(x));
...
async function publishFinal() {
    ...
    let propertyPayload = {
        ...
        status: statusInp, 
        phone: cleanPhone(activeUser.phone),
...
async function publishBuyerRequirement() {
    ...
    let phone = cleanPhone(document.getElementById('reqFormPhone')?.value.trim() || '');
            ...
            .onSnapshot(snapshot => {
                snapshot.forEach(doc => {
                    ...
                    let mappedDataNode = {
                        ...
                        phone: cleanPhone(property.phone) || "7869824155",
...
function renderProfileSectionWorkspace() {
    ...
    if (activeUser) {
        ...
        if (pPhone) pPhone.innerText = `+91 ${cleanPhone(activeUser.phone)}`;
...
// ===== BUILDER/BROKER PROFILE MANAGEMENT =====
let currentBuilderProfile = {
    id: "builder_1",
    companyName: "Premium Properties Builder",
    about: "We are a leading real estate developer building premium residential and commercial properties across India with over 20 years of experience.",
    logo: "https://placehold.co/100x100/f59e0b/0b1329?text=Builder",
    banner: "",
    phone: "+91 98765 43210",
    mapLink: "https://maps.google.com/maps?q=Bhopal,MP&output=embed",
    totalProjects: 42,
    activeListings: 18,
    soldProperties: 24,
    rating: 4.8,
    reviews: [
        { name: "Rahul Kumar", rating: 5, text: "Excellent service and transparent dealings. Highly recommended!" },
        { name: "Priya Singh", rating: 5, text: "Best builder in town. Quality construction and timely delivery." },
        { name: "Amit Patel", rating: 4, text: "Good experience. Quick response to queries." }
    ]
};

let builderProfileData = {};

function loadBuilderProfile() {
    try {
        let stored = safeStorage.getItem('mamadealing_builder_profile');
        if (stored) {
            builderProfileData = JSON.parse(stored);
            currentBuilderProfile = { ...currentBuilderProfile, ...builderProfileData };
        }
    } catch (e) {
        console.warn("Builder profile load error:", e);
    }
}

function saveBuilderProfileToStorage() {
    try {
        safeStorage.setItem('mamadealing_builder_profile', JSON.stringify(currentBuilderProfile));
    } catch (e) {
        console.warn("Builder profile save error:", e);
    }
}

function openBuilderProfile(builderId) {
    loadBuilderProfile();
    renderBuilderProfilePage();
    document.getElementById('builderBrokerProfilePage').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
}

function closeBuilderProfile() {
    document.getElementById('builderBrokerProfilePage').classList.add('hidden');
    document.body.style.overflow = 'auto';
}

function renderBuilderProfilePage() {
    // Header
    document.getElementById('builderCompanyName').innerText = currentBuilderProfile.companyName || "Builder Name";
    document.getElementById('builderAboutText').innerText = currentBuilderProfile.about || "No description available.";
    
    // Logo
    document.getElementById('builderLogoImg').src = currentBuilderProfile.logo || "https://placehold.co/100x100/f59e0b/0b1329?text=Logo";
    
    // Stats
    document.getElementById('builderTotalProjects').innerText = currentBuilderProfile.totalProjects || 0;
    document.getElementById('builderActiveListings').innerText = currentBuilderProfile.activeListings || 0;
    document.getElementById('builderSoldProperties').innerText = currentBuilderProfile.soldProperties || 0;
    document.getElementById('builderRating').innerText = currentBuilderProfile.rating || "0";
    
    // Map
    let mapLink = currentBuilderProfile.mapLink || "https://maps.google.com/maps?q=Bhopal,MP&output=embed";
    document.getElementById('builderMapIframe').src = mapLink;
    
    // Active Listings Grid
    let builderListings = database.filter(x => x.phone && cleanPhone(x.phone) === cleanPhone(currentBuilderProfile.phone) && !x.isSold);
    renderActiveListingsGrid(builderListings.slice(0, 4));
    
    // Sold Properties
    let soldListings = database.filter(x => x.phone && cleanPhone(x.phone) === cleanPhone(currentBuilderProfile.phone) && x.isSold);
    renderSoldPropertiesGrid(soldListings.slice(0, 4));
    
    // Reviews
    renderBuilderReviews();
}

function renderActiveListingsGrid(listings) {
    let container = document.getElementById('builderActiveListingsGrid');
    container.innerHTML = '';
    
    listings.forEach(prop => {
        let card = document.createElement('div');
        card.className = 'bg-slate-50 dark:bg-[#131e3a] rounded-lg overflow-hidden cursor-pointer hover:shadow-md transition';
        card.onclick = () => openPropertyDetail(prop.id);
        
        let img = prop.images && prop.images[0] ? prop.images[0] : NO_IMAGE_FALLBACK;
        card.innerHTML = `
            <div class="h-32 bg-slate-200 dark:bg-slate-700 overflow-hidden">
                <img src="${img}" alt="Property" class="w-full h-full object-cover">
            </div>
            <div class="p-2">
                <p class="text-[10px] font-bold text-slate-900 dark:text-white truncate">${prop.locality || "Unknown"}</p>
                <p class="text-xs font-bold text-brand-gold">₹ ${formatPrice(prop.price)}</p>
            </div>
        `;
        container.appendChild(card);
    });
...
function renderSoldPropertiesGrid(listings) {
    let container = document.getElementById('builderSoldPropertiesGrid');
    container.innerHTML = '';
    
    listings.forEach(prop => {
        let card = document.createElement('div');
        card.className = 'bg-slate-50 dark:bg-[#131e3a] rounded-lg overflow-hidden opacity-75 relative';
        ...
        card.innerHTML = `
            <div class="h-32 bg-slate-200 dark:bg-slate-700 overflow-hidden">
                ...
                <div class="absolute inset-0 bg-black/40 flex items-center justify-center">
                    <span class="text-white font-bold text-xs bg-red-600 px-2 py-1 rounded">SOLD</span>
                </div>
            </div>
            <div class="p-2">
                <p class="text-[10px] font-bold text-slate-900 dark:text-white truncate">${prop.locality || "Unknown"}</p>
                <p class="text-xs font-bold text-slate-500 line-through">₹ ${formatPrice(prop.price)}</p>
            </div>
        `;
        container.appendChild(card);
    });
...
function renderBuilderReviews() {
    let container = document.getElementById('builderReviewsContainer');
    container.innerHTML = '';
    
    if (!currentBuilderProfile.reviews || currentBuilderProfile.reviews.length === 0) {
        container.innerHTML = '<p class="text-sm text-slate-500 dark:text-slate-400">No reviews yet</p>';
        return;
    }
    
    currentBuilderProfile.reviews.forEach(review => {
        let reviewEl = document.createElement('div');
        reviewEl.className = 'bg-slate-50 dark:bg-[#131e3a] p-4 rounded-xl';
        
        let stars = '★'.repeat(review.rating) + '☆'.repeat(5 - review.rating);
        reviewEl.innerHTML = `
            <div class="flex justify-between items-start mb-2">
                <span class="font-bold text-sm text-slate-900 dark:text-white">${review.name}</span>
                <span class="text-xs text-brand-gold">${stars}</span>
            </div>
            <p class="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">${review.text}</p>
        `;
        container.appendChild(reviewEl);
    });
}

function openBuilderEditModal() {
    const activeUser = window.sessionUser || sessionUser;
    if (!activeUser) {
    ...
    document.getElementById('editBuilderNameInp').value = currentBuilderProfile.companyName || "";
    document.getElementById('editBuilderAboutInp').value = currentBuilderProfile.about || "";
    document.getElementById('editBuilderLogoInp').value = currentBuilderProfile.logo || "";
    document.getElementById('editBuilderMapLinkInp').value = currentBuilderProfile.mapLink || "";
    
    document.getElementById('builderEditModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
}

function closeBuilderEditModal() {
    document.getElementById('builderEditModal').classList.add('hidden');
    document.body.style.overflow = 'auto';
}

function saveBuilderProfile() {
    currentBuilderProfile.companyName = document.getElementById('editBuilderNameInp').value || currentBuilderProfile.companyName;
    currentBuilderProfile.about = document.getElementById('editBuilderAboutInp').value || currentBuilderProfile.about;
    currentBuilderProfile.logo = document.getElementById('editBuilderLogoInp').value || currentBuilderProfile.logo;
    currentBuilderProfile.mapLink = document.getElementById('editBuilderMapLinkInp').value || currentBuilderProfile.mapLink;
    
    saveBuilderProfileToStorage();
    
    if (firestoreDb) {
        try {
            let cPhone = cleanPhone(currentBuilderProfile.phone);
            firestoreDb.collection('builders').doc(cPhone).set(currentBuilderProfile, { merge: true });
        } catch (e) {
            console.warn("Builder profile sync error:", e);
        }
    }
    
    alert("✅ Profile updated successfully!");
    renderBuilderProfilePage();
    closeBuilderEditModal();
}

function callBuilder() {
    let phone = currentBuilderProfile.phone;
    if (phone) {
        window.location.href = `tel:${phone}`;
    } else {
        alert("⚠️ Phone number not available");
    }
}
...
function whatsappBuilder() {
    let phone = currentBuilderProfile.phone;
    if (phone) {
        let cleanedPhone = cleanPhone(phone).replace(/^91/, '');
        window.open(`https://wa.me/91${cleanedPhone}?text=Hi, I'm interested in your properties.`, '_blank');
    } else {
        alert("⚠️ Phone number not available");
    }
}
...
function liveChat() {
    alert("💬 Live chat feature is coming soon! You can also reach out via WhatsApp.");
}
...
// ========== NEW FEATURES: SUBSCRIPTION TIERS, REELS, OWNER PROFILES ==========

// PROPERTY LIMITS BY PLAN & ROLE
const PROPERTY_LIMITS = {
    "Free": { owner: 5, broker: 3, builder: 3 },
    "Silver": { owner: 25, broker: 15, builder: 20 },
    "Gold": { owner: 50, broker: 40, builder: 50 }
};
...
// Get valid subscription with auto-expiry check
function getValidSubscriptionPlan(phone) {
    let cPhone = cleanPhone(phone);
    const activeUser = window.sessionUser || sessionUser;
    ...
    if (activeUser && cleanPhone(activeUser.phone) === cPhone) {
        if (activeUser.subscribedPlan && activeUser.subscribedPlan !== "Free") {
            if (!activeUser.planExpiresAt || Date.now() <= Number(activeUser.planExpiresAt)) {
                ...
                activeUser.subscribedPlan = "Free";
                activeUser.planExpiresAt = null;
            }
        }
    }
...
// Set subscription with 30-day validity
function setSubscriptionPlan(phone, plan, daysValidity = SUBSCRIPTION_VALIDITY_DAYS) {
    let cPhone = cleanPhone(phone);
    const expiresAt = Date.now() + (daysValidity * 24 * 60 * 60 * 1000);
...
// Check if user can post more properties
function canPostMoreProperties() {
    const activeUser = window.sessionUser || sessionUser;
    if (!activeUser) return false;

    const plan = getValidSubscriptionPlan(activeUser.phone);
    const role = activeUser.role || "Owner";
    const limitForPlanAndRole = PROPERTY_LIMITS[plan][role.toLowerCase()] || 0;
    const currentCount = getPropertyCountForCurrentUser();

    return currentCount < limitForPlanAndRole;
}

// Get property count for current user
function getPropertyCountForCurrentUser() {
    const activeUser = window.sessionUser || sessionUser;
    if (!activeUser) return 0;
    
    const userPhone = cleanPhone(activeUser.phone);
    return database.filter(prop => cleanPhone(prop.phone) === userPhone && !isDemoProperty(prop)).length;
}

// OWNER PROFILE SYSTEM
let ownerProfilesDb = {};

function getOrCreateOwnerProfile(phone) {
    let cPhone = cleanPhone(phone);
    
    if (!ownerProfilesDb[cPhone]) {
        ownerProfilesDb[cPhone] = {
            phone: cPhone,
            role: "Owner",
            name: "",
            about: "",
            profilePhoto: NO_IMAGE_FALLBACK,
            contactPhone: cPhone,
            rating: "0",
            reviews: [],
            createdAt: Date.now()
        };
        saveOwnerProfilesToStorage();
    }
    
    return ownerProfilesDb[cPhone];
}

function saveOwnerProfilesToStorage() {
    safeStorage.setItem('mamadealing_owner_profiles', JSON.stringify(ownerProfilesDb));
}

function loadOwnerProfilesFromStorage() {
    try {
        const ownerData = safeStorage.getItem('mamadealing_owner_profiles');
        if (ownerData) ownerProfilesDb = JSON.parse(ownerData);
    } catch(e) {
        console.warn("Owner profile load error:", e);
    }
}

// REEL MANAGEMENT
function addReelToProfile(phone, videoUrl, caption = "", role = "Owner") {
    const reel = {
        id: Date.now(),
        url: videoUrl,
        caption: caption,
    ...
    if (role === "Broker") {
        let profile = currentBrokerProfile || getOrCreateBrokerProfile(phone);
        profile.reels = profile.reels || [];
        profile.reels.push(reel);
        saveBrokerProfileToStorage();
    } else if (role === "Builder") {
        let profile = currentBuilderProfile || getOrCreateBuilderProfile(phone);
        profile.reels = profile.reels || [];
        profile.reels.push(reel);
        saveBuilderProfileToStorage();
    }
...
function deleteReelFromProfile(phone, reelId, role = "Owner") {
    if (role === "Broker") {
        let profile = currentBrokerProfile || getOrCreateBrokerProfile(phone);
        if (profile.reels) {
            profile.reels = profile.reels.filter(r => r.id !== reelId);
            saveBrokerProfileToStorage();
        }
    } else if (role === "Builder") {
        let profile = currentBuilderProfile || getOrCreateBuilderProfile(phone);
...
function updateProfilePhoto(phone, photoUrl, role) {
    if (role === "Broker") {
        let profile = currentBrokerProfile || getOrCreateBrokerProfile(phone);
        ...
        let profile = getOrCreateOwnerProfile(phone);
        profile.profilePhoto = photoUrl;
        saveOwnerProfilesToStorage();
    }
}
...
// REVIEW SYSTEM
function addReviewToProfile(phone, reviewerName, rating, reviewText, role) {
    const review = {
        id: Date.now(),
        name: reviewerName,
        rating: rating,
...
function showSubscriptionModal() {
    ...
    const role = activeUser.role || "Owner";
    const currentCount = getPropertyCountForCurrentUser();
    const limit = PROPERTY_LIMITS[plan][role.toLowerCase()] || 0;

    document.getElementById('currentPlanDisplay').innerText = plan || "Free";
    document.getElementById('propertyCountDisplay').innerText = currentCount;
    document.getElementById('propertyLimitDisplay').innerText = limit;
    
    const percentage = limit > 0 ? (currentCount / limit) * 100 : 0;
    if (document.getElementById('propertyLimitFill')) {
        document.getElementById('propertyLimitFill').style.width = Math.min(percentage, 100) + '%';
    }

    if (document.getElementById('platformSubscriptionPlansModal')) {
        openFeature('platformSubscriptionPlansModal');
    } else {
        alert('Current Plan: ' + plan + '\nListings: ' + currentCount + '/' + limit);
    }
}

function initiatePurchase(planName, price) {
    const activeUser = window.sessionUser || sessionUser;
    if (!activeUser) {
        alert("🔒 Please login first");
        return;
    ...
    if (typeof Razorpay !== 'undefined') {
        const options = {
            ...
            currency: "INR",
            name: "Mama Dealing",
            description: `${planName} Plan - ₹${price}`,
            image: "logo-light.png",
            handler: function(response) {
                setSubscriptionPlan(activeUser.phone, planName, SUBSCRIPTION_VALIDITY_DAYS);
