...
// Updated BACKEND_BASE_URL to point to mamadealing-backend-1
...
// Price formatting system for clean display like Lakhs and Crores (99acres layout style)
...
function initializeApp() {
    ...
    // Keyboard Left/Right Arrow listeners for changing photos in details modal
...
// Strictly default theme initialization logic set to Light Mode
function initTheme() {
    ...
    Object.keys(mobileTabs).forEach(k => {
        ...
        if (mEl) {
            if (k === tabId) {
                ...
                const icon = mEl.querySelector('i');
                if (icon) icon.className = icon.className.replace('text-brand-gold', 'text-slate-505');
            }
        }
    });
...
// Locks background scrolling to prevent duplicate double scrollbars in Details view
function openPropertyDetailsModal(id) {
    try {
        ...
        // LOCK main body page scrolling to completely prevent duplicate double scrollbars
...
// Touch event handlers for Mobile Slide Swipe photo switching
...
function handleDetailSwipeGesture() {
    let threshold = 55; // swipe distance in pixels
    if (detailTouchStartX - detailTouchEndX > threshold) {
        changeDetailPhoto(1); // Swiped Left -> Next Image
    } else if (detailTouchEndX - detailTouchStartX > threshold) {
        changeDetailPhoto(-1); // Swiped Right -> Previous Image
    }
}
...
function closePropertyDetailModalBlock() {
    ...
    // UNLOCK main body page scrolling when modal is dismissed
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    allDistDiv.className = "p-2.5 border-b border-slate-101 dark:border-slate-808 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer text-slate-900 dark:text-white font-bold flex justify-between items-center simple-district-item text-[11px] bg-white dark:bg-[#131e3a]";
...
// Fixed geography selector parameters mapping to active browse grid
        ...
        authComp.innerHTML = `
            <div class="relative inline-block text-left" id="desktopProfileDropdownWrapper">
                ...
                <div id="desktopProfileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[#131e3a] rounded-lg shadow-lg border border-slate-200 dark:border-slate-800 py-1 z-50">
                    <button onclick="switchTab('profile'); goBackToProfileDashboard();" class="block w-full text-left px-4 py-2 text-xs font-bold text-slate-700 dark:text-white hover:bg-slate-101 dark:hover:bg-slate-801">My Dashboard</button>
                    <button onclick="logoutAgent(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-red-600 hover:bg-slate-101 dark:hover:bg-slate-801">Logout</button>
                </div>
            </div>
        `;
    }
...
function startListeningToMyChats() {
    if (!sessionUser || !firestoreDb) return;
...
// Closes open user profile dropdown if click occurs outside layout boundary
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-707 dark:text-slate-202 flex items-center justify-center border border-slate-300 dark:border-slate-707 shadow-sm transition-all icon-box";
            if (txt) {
    ...
    if (btn) {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-500 text-slate-955 flex items-center justify-center border-2 border-amber-500 scale-110 shadow-md icon-box transition-all";
            if (txt) {
...
function applySearchPillType(type, btn) {
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-200 dark:bg-[#131e3a] text-slate-707 dark:text-slate-202 flex items-center justify-center border border-slate-300 dark:border-slate-707 shadow-sm transition-all icon-box";
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-707 dark:text-slate-300 uppercase tracking-wide";
            }
        } else { 
...
function openOlxChatSimulation() {
            ...
            .onSnapshot(doc => {
                ...
                if (data && data.messages) {
                    ...
                    firestoreDb.collection('chats').doc(chatId).set({
                        ...
                        buyerPhone: sessionUser.phone,
                        buyerName: sessionUser.firmName || "Interested Buyer",
    ...
    // Set standard buyer message sender callback
...
function openOlxChatSimulationFromList(propertyId, buyerPhone, sellerName) {
    ...
    if (sendBtn) {
        sendBtn.onclick = () => {
            ...
            const isBuyer = String(buyerPhone) === String(sessionUser.phone);
            const userMessage = {
...
function renderRealtimeMessages(messages) {
    ...
    messages.forEach(m => {
        ...
        // Match standard sender logic based on active profile context
        if (m.sender === 'user') {
            isMe = (String(targetDetailItemInstance?.phone) !== String(sessionUser?.phone));
...
function sendUserChatMessageSimulation() {
    // Overridden dynamically inside initialization routines.
}
...
function render() {
    ...
    if (hasNewGrids) {
        ...
        if(premiumItems.length === 0) {
            featuredGrid.innerHTML = `<div class="col-span-full py-4 text-center text-[11px] font-semibold text-slate-400">No featured projects in this region.</div>`;
        } else {
...
function createPropertyCardNode(x) {
    ...
    div.innerHTML = `
        <div class="relative h-64 sm:h-72 bg-slate-900 overflow-hidden">
            ...
            <span class="absolute top-9 left-2.5 bg-amber-500 text-slate-955 font-black text-[7px] uppercase tracking-widest px-2 py-0.5 rounded shadow-sm z-10">${x.status || 'READY TO MOVE'}</span>
            ${premiumVisualBadge}
            ...
            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-955 via-slate-900/60 to-transparent p-3 pt-10">
        ...
        <div class="p-3.5 space-y-2 font-bold">
            ...
            <p class="text-[10px] text-slate-600 dark:text-slate-300 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-brand-gold"></i> ${x.locality || ""}, ${x.city || ""}</p>
            ...
            <div class="pt-2 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                ...
                <span class="text-[9px] bg-slate-101 dark:bg-slate-800 text-slate-707 dark:text-white px-2 py-0.5 rounded-md font-bold">Details →</span>
            </div>
        </div>`;
    return div;
}
...
function renderPaginationControls(totalPages) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-slate-808 text-[10px] font-bold cursor-pointer ${currentPropertiesPage === i ? 'bg-[#0b1329] text-brand-gold' : 'bg-white text-slate-700 dark:text-white'}`;
        btn.onclick = () => { currentPropertiesPage = i; render(); };
        btn.innerText = i;
        container.appendChild(btn);
    }
    let nextBtn = document.createElement('button');
    nextBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-[#0b1329] text-[10px] font-bold bg-white dark:bg-[#0b1329] text-slate-500 dark:text-white hover:bg-slate-50 cursor-pointer";
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        ...
        activeChatsStorage.forEach(chat => {
            let displayName = String(chat.buyerPhone) === String(sessionUser.phone) 
            ...
            htmlContent += `
                <div onclick="openOlxChatSimulationFromList('${chat.propertyId}', '${chat.buyerPhone}', '${displayName}')" class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-202 dark:border-slate-707 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm hover:border-brand-gold">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-500 dark:text-slate-303 font-semibold">Unit: ${chat.propertyTitle || 'Property'}</p>
                    </div>
...
function renderMyPropertiesManagementWorkspace() {
    ...
    container.innerHTML = "";
    if (!sessionUser) {
        container.innerHTML = `<p class="text-xs font-semibold text-slate-550 py-2 text-center">Please log in to manage your assets.</p>`;
        return;
    }
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <button onclick="openEditPropertyModal('${x.id}')" class="bg-amber-100 hover:bg-amber-202 text-amber-808 text-[10px] font-extrabold uppercase px-3.5 py-1.5 rounded-lg border border-amber-202 w-full md:w-auto">Edit</button>
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-55 hover:bg-red-101 text-red-707 dark:bg-red-955/20 dark:text-red-404 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-101 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
...
async function publishFinal() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert("⚠️ Saved locally, but failed to synchronize with backend server.");
        }
    } catch (err) {
            ...
            <div class="space-y-3 font-bold">
                <div class="flex justify-between items-start gap-1">
                    ...
                    <div class="text-right">
                        <p class="text-xs text-slate-505 dark:text-slate-400 uppercase">Max Budget</p>
                ...
                <div class="pt-2 border-t border-slate-101 dark:border-slate-800">
                    <p class="text-[9px] text-slate-500 dark:text-slate-404 uppercase mb-1">Requirement Details</p>
                    <p class="text-[11px] text-slate-600 dark:text-slate-200 leading-relaxed font-semibold">${x.description}</p>
                </div>
            </div>

            <div class="pt-3 border-t border-slate-101 dark:border-slate-800 flex justify-between items-center text-[10px] font-bold">
                <div>
                ...
                <div class="flex gap-2">
                    <a href="tel:${x.phone}" class="bg-slate-900 dark:bg-slate-800 hover:bg-slate-850 text-white px-3 py-1.5 rounded-lg border dark:border-slate-707 flex items-center gap-1"><i class="ph-fill ph-phone text-brand-gold"></i> Dial</a>
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-amber-101 text-amber-808 hover:bg-amber-202 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    } else {
...
function renderProfileSectionWorkspace() {
    ...
    if (sessionUser) {
        ...
        if (pBadgeSlot) {
            pBadgeSlot.innerHTML = `<span class="bg-blue-101 text-blue-808 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider"><i class="ph-fill ph-seal-check"></i> ${sessionUser.role}</span>`;
        }
    } else {
