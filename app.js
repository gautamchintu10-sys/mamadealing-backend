...
let servicesDatabase = [
    { id: 1, category: "Legal", title: "Sharma & Associates Title Consultants", phone: "9827011223", specialty: "Registry, Title Clearance, RERA Expert", rating: 4.9, reviews: 24, verified: true },
    { id: 2, category: "Interior", title: "The Decore & Spaces Design", phone: "9829023412", specialty: "Premium 3D Modular Kitchen & Wardrobe Experts", rating: 4.8, reviews: 31, verified: true },
    { id: 3, category: "Construction", title: "Buildwell Infrastructure Developers", phone: "7000123456", specialty: "Turnkey Grey Structure & Building Contractor", rating: 4.7, reviews: 14, verified: true }
];
...
// DYNAMIC BACKEND DETECTOR FOR ROBUST API ROUTING
...
function initGeographyMenus() {
    ...
    Object.keys(GEOGRAPHY_MATRIX).sort().forEach(st => {
        if (sMain) sMain.innerHTML += `<option value="${st}">${st}</option>`;
...
function loadMainCitiesMenu() {
    ...
    if(state !== "All" && GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(ci => {
            cMain.innerHTML += `<option value="${ci}">${ci}</option>`;
        });
    }
}
...
function triggerNearMeLocationEngine() {
    ...
    if (scanRadiusActive) {
        ...
        if (nearMeBtn) {
            ...
            nearMeBtn.innerHTML = `<i class="ph-bold ph-crosshair text-sm animate-pulse"></i> Near Me`;
        }
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            if (nearMeBtn) {
                ...
                nearMeBtn.innerHTML = `<i class="ph-bold ph-check text-sm animate-bounce"></i> Near Active`;
            }
...
function render() {
    ...
    filtered.forEach(x => {
        ...
        div.className = "p-card animate-slide-up relative bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300";
...
function switchTab(tabId) {
    ...
    const viewIds = ['main-properties-view', 'main-reels-view', 'main-services-view', 'main-profile-view', 'main-about-view'];
    viewIds.forEach(id => {
    ...
    if(tabId === "properties") {
        ...
        if (navBar) navBar.classList.remove('hidden');
        let el = document.getElementById('main-about-view');
        if (el) el.classList.remove('hidden');
    }
}
...
function renderReelsList() {
    ...
    reelsData.forEach((x, index) => {
        ...
        div.innerHTML = `
            <video src="${x.video}" loop muted playsinline preload="auto" onclick="toggleReelPlaybackEvent(this)"></video>
...
function renderB2bServicesHub() {
    ...
    filteredServices.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-white p-5 rounded-2xl border border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-slide-up shadow-sm";
        ...
        div.innerHTML = `
            <div class="space-y-1">
                <div class="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span class="${categoryColorBadge} border text-[9px] font-black px-2 py-0.5 rounded uppercase flex items-center gap-0.5"><i class="ph-fill ph-seal-check"></i> ${x.category} Expert</span>
                </div>
            ...
            <a href="tel:${x.phone}" class="w-full sm:w-auto bg-slate-900 text-white text-center font-black text-xs uppercase px-6 py-3.5 rounded-xl flex items-center justify-center gap-1.5 shadow-sm hover:bg-slate-800 transition-colors"><i class="ph-fill ph-phone"></i> Dial Connect</a>
        `;
        grid.appendChild(div);
    });
}
...
function renderProfileSectionWorkspace() {
    ...
    if(!sessionUser) {
        ...
        if (pCounter) pCounter.innerText = "Total Free Post Used: 0 / 3";
    } else {
        ...
        if (pCounter) pCounter.innerText = `Total Free Post Used: ${userPostsCount} / 3`;
    }
...
function triggerDirectPostFlow() {
    console.log("Post Listing Free process triggered safely.");
    try {
        if(!sessionUser) {
            console.log("No authenticated credentials active, opening login screen.");
            openFeature('loginModal');
            return;
        }
        ...
        try {
            ...
            console.warn("Soft issue caught on rendering previews:", previewErr);
        }

        try {
            moveStep(1);
        } catch (stepErr) {
            console.warn("Soft issue caught on step indexing:", stepErr);
        }

        console.log("Launching core post Property modal.");
        openFeature('postProperty');
    } catch (globalFlowErr) {
        console.error("Global flow error caught. Enforcing emergency modal reveal:", globalFlowErr);
        openFeature('postProperty');
    }
}
...
async function triggerOtpRequestSequence() {
    ...
    try {
        ...
        alert("⚠️ SMS Gateway could not be reached.");
    }
}
...
function verifyMockOtpFrameworkSequence() {
    // Deprecated for Real SMS Flow (Kept as fallback just in case)
    verifyRealOtpSequence();
}
...
function logoutAgent() {
    ...
    alert("🔒 Active cryptographic credentials token purged cleanly. Session terminated.");
...
function submitNewB2bBusinessForm() {
    ...
    if(!t || !p || !s) { 
        alert("⚠️ Complete details required to list business on our directory."); 
        return; 
    }
    
    closeFeature('addB2bModal');
    let pendingListing = { id: Date.now(), category: c, title: t, phone: p, specialty: s, rating: 5.0, reviews: 1, verified: true };
