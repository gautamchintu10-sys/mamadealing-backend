...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 flex items-center justify-center border border-slate-300 dark:border-slate-707 shadow-sm transition-all icon-box";
            if (txt) {
...
function applySearchPillType(type, btn) {
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-200 dark:bg-[#131e3a] text-slate-700 dark:text-slate-202 flex items-center justify-center border border-slate-300 dark:border-slate-707 shadow-sm transition-all icon-box";
            if (txt) {
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                ...
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 hover:bg-red-101 text-red-700 dark:bg-red-955/20 dark:text-red-400 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-100 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
        ...
        card.innerHTML = `
            <div class="space-y-3 font-bold">
                ...
                <div class="pt-2 border-t border-slate-100 dark:border-slate-800">
                    <p class="text-[9px] text-slate-400 dark:text-slate-404 uppercase mb-1">Requirement Details</p>
