...
// Fallback listings kept empty to load only active live listings
let database = [];
    ...
    Object.keys(mobileTabs).forEach(k => {
        ...
        if (mEl) {
            if (k === tabId) {
                ...
                mEl.className = "flex flex-col items-center gap-0.5 cursor-pointer text-slate-500 dark:text-slate-404 w-11";
                const icon = mEl.querySelector('i');
    ...
    try {
        ...
        if (videoTab && videoPlayer) {
            if (item.video && item.video !== "") {
                ...
                videoPlayer.load(); // Forces reload across safari/chrome decoders
            } else {
...
function switchDetailMediaMode(mode) {
    ...
    if (mode === 'photos') {
        ...
        if (videoPlayer) {
            videoPlayer.load(); // Force decoder play reload
...
function updateLightboxUI() {
    ...
    if (isVideoActive && item.video) {
        ...
        if (lightboxVid) {
            ...
            lightboxVid.load(); // Force decoder play
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    if (!tray) return;
    if (!state) {
        tray.innerHTML = `<p class="text-slate-500 dark:text-white text-center py-6 font-semibold text-xs font-bold">Please select state first</p>`;
        return;
    }
    ...
    if (GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(dist => {
            ...
            div.className = "p-2.5 border-b hover:bg-slate-101 dark:hover:bg-slate-800 cursor-pointer text-slate-707 dark:text-white simple-district-item text-[11px] font-semibold bg-white dark:bg-[#131e3a] font-bold";
            div.innerText = dist;
...
// REST OF THE CODE WAS NOT CHANGED BY THE USER - KEEPING EVERYTHING ORIGINAL BUT UPDATING SHARE LINKS TO MATCH AND PREVENT ERRORS
    ...
    if (authComp && activeUser) {
        authComp.innerHTML = `
            <div class="relative inline-block text-left" id="desktopProfileDropdownWrapper">
                ...
                <div id="desktopProfileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[#131e3a] rounded-lg shadow-lg border border-slate-202 dark:border-slate-808 py-1 z-50">
                    <button onclick="switchTab('profile'); goBackToProfileDashboard();" class="block w-full text-left px-4 py-2 text-xs font-bold text-slate-707 dark:text-white hover:bg-slate-101 dark:hover:bg-slate-808">My Dashboard</button>
                    <button onclick="logoutAgent(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-red-600 hover:bg-slate-101 dark:hover:bg-slate-808">Logout</button>
                </div>
            </div>
        `;
    }
...
function startListeningToMyChats() {
    try {
        ...
        const handleChatsUpdate = (querySnapshot) => {
            let isUnreadTriggered = false;
            querySnapshot.forEach(doc => {
                const data = doc.data();
                const oldChatInstance = allChatsMap.get(doc.id);
                if (oldChatInstance && data.messages && data.messages.length > oldChatInstance.messages.length) {
                    const lastMsgNode = data.messages[data.messages.length - 1];
                    if (lastMsgNode.sender !== 'user' && lastMsgNode.sender !== 'seller_me') {
                        isUnreadTriggered = true;
                    }
                }
            ...
            if (isUnreadTriggered) {
                const mBadge = document.getElementById('chatNotificationBadgeMobile');
                const dBadge = document.getElementById('chatNotificationBadgeDesktop');
                if (mBadge) mBadge.classList.remove('hidden');
...
// REST OF CODE REMAINED THE SAME, JUST UPDATING THE DYNAMIC SHARE GENERATOR IN APP.JS
...
function applySearchPillTab(cat, btn) {
    ...
    document.querySelectorAll('.custom-search-tab').forEach(b => {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-slate-202 dark:bg-[#131e3a] text-slate-707 dark:text-slate-202 flex items-center justify-center border border-slate-300 dark:border-slate-707 shadow-sm transition-all icon-box";
            if (txt) {
                txt.className = "text-[10px] font-bold text-slate-707 dark:text-slate-303 uppercase tracking-wide";
            }
        } else { 
...
function renderReelsList(filterQuery = "") {
    ...
    if(reelsData.length === 0) {
        container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-center p-6 space-y-2"><i class="ph ph-video-camera-slash text-4xl text-slate-505 dark:text-white font-bold"></i><p class="text-xs font-semibold text-slate-500 dark:text-white">No matching reels found.</p></div>`;
        return;
    }
...
// 🌐 UPDATED WITH DYNAMIC BACKEND EMBEDDING CAPABILITIES (RENDER CRAWLER FRIENDLY ENDPOINT)
function openShareToolkitChannel(id) {
    let item = database.find(x => String(x.id) === String(id));
    if (!item) return;
    currentlyTargetedShareNodeData = item;

    // Use our server's redirection routing designed to generate dynamic SVG meta cards
    ...
    // Update frontend preview visual card parameters to match real metadata
    for (let i = 1; i <= 4; i++) {
...
function renderRealtimeMessages(messages) {
    ...
    messages.forEach(m => {
        ...
        let bubbleClass = isMe 
            ...
            : "bg-slate-202 dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-2xl rounded-tl-none px-3.5 py-2 shadow-sm";
        
        let checkmarkHtml = isMe 
            ? `<i class="ph-fill ph-checks text-[12px] text-emerald-404 ml-1.5 align-middle"></i>` 
            : '';
...
function sendUserChatMessageSimulation() {
    // Bound dynamically via inputs inside main render loop listeners
}
...
function render() {
    ...
    if (hasNewGrids) {
        ...
        if(premiumItems.length === 0) {
            featuredGrid.innerHTML = `<div class="col-span-full py-4 text-center text-[11px] font-semibold text-slate-404">No featured properties in this region.</div>`;
        } else {
        ...
        if(plotItems.length === 0) {
            plotsGrid.innerHTML = `<div class="col-span-full py-4 text-center text-[11px] font-semibold text-slate-404">No plots registered in this territory.</div>`;
        } else {
        ...
        if(recentStandardItems.length === 0) {
            recentGrid.innerHTML = `<div class="col-span-full py-12 text-center text-xs font-bold text-slate-404 dark:text-white">Zero recent properties found.</div>`;
...
function createPropertyCardNode(x) {
    ...
    let dynamicFirmLabelStr = x.firmName ? `<p class="text-[10px] text-slate-505 dark:text-slate-300 font-extrabold truncate mt-0.5"><i class="ph-fill ph-buildings inline text-brand-gold"></i> ${x.firmName}</p>` : `<p class="text-[10px] text-brand-gold font-extrabold mt-0.5"><i class="ph-fill ph-seal-check inline"></i> Verified Owner</p>`;
        ...
        <div class="p-3.5 space-y-2 font-bold">
            <div class="flex justify-between items-start gap-1">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <button onclick="toggleBookmarkClientState('${x.id}', this); event.stopPropagation();" class="text-lg ${x.saved ? 'text-amber-500 ph-fill font-bold' : 'text-slate-303 dark:text-slate-300 ph'} ph-bookmark"></button>
                </div>
            </div>
            <p class="text-[10px] text-slate-606 dark:text-slate-300 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-brand-gold"></i> ${x.locality || ""}, ${x.city || ""}</p>
            
            <div class="pt-2 border-t border-slate-101 dark:border-slate-808 flex justify-between items-center">
                <div>${dynamicFirmLabelStr}</div>
                <span class="text-[9px] bg-slate-101 dark:bg-slate-808 text-slate-707 dark:text-white px-2 py-0.5 rounded-md font-bold">Details →</span>
            </div>
        </div>`;
    return div;
}
...
function renderPaginationControls(totalPages) {
    ...
    prevBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-808 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-707 dark:text-white hover:bg-slate-50 cursor-pointer";
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-808 text-[10px] font-bold cursor-pointer ${currentPropertiesPage === i ? 'bg-[#0b1329] text-brand-gold' : 'bg-white text-slate-707 dark:text-white'}`;
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        ...
        activeChatsStorage.forEach(chat => {
            ...
            htmlContent += `
                <div onclick="openOlxChatSimulationFromList('${chat.propertyId}', '${chat.buyerPhone}', '${displayName}')" class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-202 dark:border-slate-707 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm hover:border-brand-gold">
                    <div>
                        ...
                        <p class="text-[9px] text-slate-505 dark:text-slate-300 font-semibold">Unit: ${chat.propertyTitle || 'Property'}</p>
                    </div>
...
function renderSavedBookmarksDashboard() {
    ...
    grid.innerHTML = "";
    if(saved.length === 0) {
        grid.innerHTML = `<p class="text-xs font-semibold text-slate-505 col-span-full py-4 text-center">No properties saved yet.</p>`;
        return;
    }
    saved.forEach(x => {
        let div = document.createElement('div');
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <button onclick="openEditPropertyModal('${x.id}')" class="bg-amber-100 hover:bg-amber-202 text-amber-808 text-[10px] font-extrabold uppercase px-3.5 py-1.5 rounded-lg border border-amber-200 w-full md:w-auto">Edit</button>
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 hover:bg-red-101 text-red-700 dark:bg-red-900/20 dark:text-red-404 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-101 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
...
function moveStep(st) {
    ...
    stages.forEach(s => {
        ...
        if (ind) {
            ind.className = parseInt(s) <= st ? "w-3.5 h-1 bg-[#0b1329] dark:bg-white rounded-full transition-all" : "w-3.5 h-1 bg-slate-202 dark:bg-slate-808 rounded-full transition-all font-bold";
        }
    });
...
function switchBrowseView(view) {
    ...
    if (view === 'properties') {
        ...
        reqBtn.className = "px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all text-slate-606 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white";
    } else {
        ...
        propBtn.className = "px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all text-slate-606 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white";
    ...
    requirementsDatabase.forEach(x => {
        ...
        card.innerHTML = `
            <div class="space-y-3 font-bold">
                ...
                <p class="text-[10px] text-slate-606 dark:text-slate-300 font-semibold flex items-center gap-0.5"><i class="ph-fill ph-map-pin text-brand-gold"></i> Preferred Location: ${x.locality}, ${x.city}</p>
                
                <div class="pt-2 border-t border-slate-202 dark:border-slate-808">
                    <p class="text-[9px] text-slate-404 dark:text-slate-404 uppercase mb-1">Requirement Details</p>
                    <p class="text-[11px] text-slate-606 dark:text-slate-202 leading-relaxed font-semibold">${x.description}</p>
                </div>
            </div>

            <div class="pt-3 border-t border-slate-202 dark:border-slate-808 flex justify-between items-center text-[10px] font-bold">
                <div>
                ...
                <div class="flex gap-2">
                    ...
                    <a href="https://wa.me/91${x.phone}?text=Hi%20${encodeURIComponent(x.name)},%20I%20saw%20your%20requirement%20on%20Mama%20Dealing..." target="_blank" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg flex items-center gap-1"><i class="ph-fill ph-whatsapp-logo"></i> WhatsApp</a>
                </div>
            </div>`;
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-amber-100 text-amber-808 hover:bg-amber-202 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    } else {
...
function renderProfileSectionWorkspace() {
    ...
    if (activeUser) {
        ...
        if (pBadgeSlot) {
            pBadgeSlot.innerHTML = `<span class="bg-blue-101 text-blue-800 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider"><i class="ph-fill ph-seal-check"></i> ${activeUser.role}</span>`;
        }
    } else {
