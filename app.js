...
function initializeApp() {
    ...
    // लॉगिन निरंतरता (Persistent Login) बनाए रखने के लिए लोकल स्टोरेज की जाँच करें
...
function switchTab(tabId) {
    ...
    // इंस्टाग्राम रील्स की तरह फुल स्क्रीन लागू करने के लिए क्लास टॉगल करें
    if (tabId === 'reels') {
...
function switchDetailMediaMode(mode) {
    ...
    if (mode === 'photos') {
        ...
        if (tabVideo) tabVideo.className = "px-2.5 py-1 rounded text-[9px] font-bold uppercase tracking-wider text-slate-950 bg-[#f59e0b] transition-all flex items-center gap-0.5";
        ...
        authComp.innerHTML = `
            <div class="relative inline-block text-left" id="desktopProfileDropdownWrapper">
                ...
                <div id="desktopProfileDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[#131e3a] rounded-lg shadow-lg border border-slate-200 dark:border-slate-800 py-1 z-50">
                    <button onclick="switchTab('profile'); goBackToProfileDashboard(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-slate-700 dark:text-white hover:bg-slate-101 dark:hover:bg-slate-800">My Dashboard</button>
                    <button onclick="logoutAgent(); closeDesktopProfileDropdown();" class="block w-full text-left px-4 py-2 text-xs font-bold text-red-600 hover:bg-slate-101 dark:hover:bg-slate-800">Logout</button>
                </div>
            </div>
        `;
    }
...
function applySearchPillTab(cat, btn) {
    ...
    if (btn) {
        ...
        if (iconBox) { 
            iconBox.className = "w-12 h-12 rounded-full bg-amber-500 text-slate-955 flex items-center justify-center border-2 border-amber-500 scale-110 shadow-md transition-all icon-box";
            if (txt) {
    ...
    pageItems.forEach(x => {
        ...
        div.innerHTML = `
            <div class="relative h-64 sm:h-72 bg-slate-900 overflow-hidden">
                ...
                <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-950 via-slate-900/60 to-transparent p-3 pt-8">
            ...
            <div class="p-3 space-y-1.5 font-bold">
                ...
                <p class="text-[10px] text-slate-606 dark:text-slate-300 font-semibold flex items-center gap-0.5 truncate"><i class="ph-fill ph-map-pin text-slate-404"></i> ${x.locality}, ${x.city}</p>
...
function renderPaginationControls(totalPages) {
    ...
    prevBtn.className = "px-2.5 py-1.5 rounded-lg border border-slate-202 dark:border-slate-800 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-707 dark:text-white hover:bg-slate-50 cursor-pointer";
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        ...
        activeChatsStorage.forEach(chat => {
            htmlContent += `
                <div onclick="openOlxChatSimulation('${chat.propertyId}')" class="p-3 bg-slate-50 dark:bg-slate-800 border border-slate-202 dark:border-slate-700 rounded-xl cursor-pointer transition-all flex justify-between items-center shadow-sm">
                    <div>
...
function renderSavedBookmarksDashboard() {
    ...
    saved.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-slate-55 dark:bg-slate-800 border border-slate-202 dark:border-slate-800 p-3 rounded-xl flex justify-between items-center cursor-pointer font-bold";
    ...
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-2 w-full md:w-auto">
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 text-red-700 dark:bg-red-955/20 dark:text-red-404 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-100 dark:border-red-900/30 w-full md:w-auto">Delete</button>
            </div>`;
        container.appendChild(div);
    });
}
...
function logoutAgent() {
    ...
    if (authComp) {
        authComp.innerHTML = `<button onclick="openFeature('loginModal')" class="bg-white hover:bg-slate-101 dark:bg-slate-800 text-slate-900 dark:text-white px-3.5 h-9 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center border border-slate-202 dark:border-slate-700 shadow-sm">Login</button>`;
    }
    renderProfileSectionWorkspace();
    switchTab('properties');
    updateMobileProfileTabLabel();
...
function renderProfileSectionWorkspace() {
    ...
    if (sessionUser) {
        ...
        if (pBadgeSlot) {
            pBadgeSlot.innerHTML = `<span class="bg-blue-100 text-blue-800 text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider"><i class="ph-fill ph-seal-check"></i> ${sessionUser.role}</span>`;
        }
    } else {
