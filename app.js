...
// डेमो और टेस्ट प्रॉपर्टीज को ब्लॉक करने की सूची
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    if (!tray) return;
    if (!state) {
        tray.innerHTML = `<p class="text-slate-555 dark:text-white text-center py-6 font-bold text-xs">Please select state first</p>`;
        return;
    }
...
function switchProfileTab(panelId) {
    let dashboardMenu = document.getElementById('profileMainDashboardState');
    let subPanelView = document.getElementById('profileSubPanelViewState');
    // Hide the main dashboard, show the sub-panel view
...
function renderMyPropertiesManagementWorkspace() {
    ...
    let myProperties = database.filter(x => String(x.phone) === String(activeUser.phone) && !isDemoProperty(x));
...
function switchBrowseView(view) {
    ...
    if (view === 'properties') {
        ...
        propBtn.className = "px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all text-slate-505 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white";
        ...
        if (propertiesArray.length > 0) {
            propertiesArray.forEach(property => {
                try {
                    ...
                    if (isDemoProperty(mappedDataNode)) {
                        return; 
                    }
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveBtn.className = "bg-amber-100 dark:bg-amber-955/40 text-amber-800 dark:text-amber-404 hover:bg-amber-202 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    } else {
        saveIcon.className = "ph ph-bookmark text-slate-500 dark:text-slate-400 text-sm";
        saveText.innerText = "Save";
        saveBtn.className = "bg-slate-101 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 font-bold";
    }
}
