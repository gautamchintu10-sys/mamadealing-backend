...
// Demo property database has been cleaned
let database = [];
...
function initializeApp() {
    ...
    // Dynamic Visual Viewport adaptation to resolve Mobile keyboard overlay layout distortion
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    allDistDiv.className = "p-2.5 border-b border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer text-slate-900 dark:text-white font-bold flex justify-between items-center simple-district-item text-[11px] bg-white dark:bg-[#131e3a]";
    ...
    if (GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(dist => {
            ...
            div.className = "p-2.5 border-b border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer text-slate-700 dark:text-white simple-district-item text-[11px] bg-white dark:bg-[#131e3a] font-bold";
            div.innerText = dist;
        ...
        const handleChatsUpdate = (querySnapshot) => {
            ...
            querySnapshot.forEach(doc => {
                ...
                if (oldChatInstance && data.messages && data.messages.length > oldChatInstance.messages.length) {
                    ...
                    if (myPhoneVal === buyerPhoneVal && lastMsgNode.sender === 'seller') {
                        isUnreadTriggered = true;
                    }
                    else if (myPhoneVal === sellerPhoneVal && lastMsgNode.sender === 'user') {
                        isUnreadTriggered = true;
                    }
                }
...
async function triggerOfficialWhatsAppTemplateSend() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert("⚠️ Meta API returned an error. Please verify API settings.");
        }
    } catch (err) {
        alert("⚠️ Failed to transmit flyer card via WhatsApp API.");
        console.error(err);
    }
}
...
function sendUserChatMessageSimulation() {
    // Dynamically attached within listeners
}
...
function renderSavedBookmarksDashboard() {
    ...
    grid.innerHTML = "";
    if(saved.length === 0) {
        grid.innerHTML = `<p class="text-xs font-semibold text-slate-500 col-span-full py-4 text-center">No properties saved yet.</p>`;
        return;
    }
    saved.forEach(x => {
        let div = document.createElement('div');
...
function updateRecentlyViewedTray() {
    ...
    if (recentItems.length === 0) {
        container.innerHTML = `<p class="text-xs text-slate-550 col-span-full py-4 text-center font-semibold">No recently viewed listings.</p>`;
        return;
    }
