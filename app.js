...
function switchTab(tabId) {
    ...
    Object.keys(mobileTabs).forEach(k => {
        ...
        if (mEl) {
            if (k === tabId) {
                mEl.classList.add('text-brand-gold');
                mEl.classList.remove('text-slate-400');
            } else {
                mEl.classList.remove('text-brand-gold');
...
function loadDistrictsForSimpleDropdown(state) {
    ...
    if (!state) {
        tray.innerHTML = `<p class="text-slate-400 dark:text-white text-center py-6 font-semibold text-xs">Please select state above first</p>`;
        return;
    }
    ...
    allDistDiv.className = "p-2.5 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer text-slate-950 dark:text-white font-bold flex justify-between items-center simple-district-item text-[11px] bg-white dark:bg-[#131e3a]";
    ...
    if (GEOGRAPHY_MATRIX[state]) {
        GEOGRAPHY_MATRIX[state].sort().forEach(dist => {
            ...
            div.className = "p-2.5 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer text-slate-700 dark:text-white simple-district-item text-[11px] font-semibold bg-white dark:bg-[#131e3a]";
            div.innerText = dist;
...
function toggleReelsMuteState() {
    ...
    icons.forEach(icon => {
        if (isReelsMuted) {
            icon.className = "ph-bold ph-speaker-slash text-base text-red-400 reels-mute-icon";
        } else {
            icon.className = "ph-bold ph-speaker-high text-base text-emerald-400 reels-mute-icon";
        }
    });
}
...
function renderReelsList(filterQuery = "") {
    ...
    if(reelsData.length === 0) {
        container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-center p-6 space-y-2">
            <i class="ph ph-video-camera-slash text-4xl text-slate-550 dark:text-white"></i>
            <p class="text-xs font-semibold text-slate-400 dark:text-white">No matching property short reels found.</p>
        </div>`;
        return;
    }
        ...
        div.innerHTML = `
            ...
            <div class="absolute bottom-16 right-3 flex flex-col items-center gap-3 z-40">
                ...
                <button onclick="toggleBookmarkClientState('${x.id}', this); event.stopPropagation();" class="w-10 h-10 bg-black/50 backdrop-blur-md rounded-full flex flex-col items-center justify-center text-white active:scale-95 transition-all shadow-xl">
                    <i class="${x.saved ? 'ph-fill ph-bookmark text-amber-404 text-base' : 'ph ph-bookmark text-base text-white'}"></i>
                </button>
...
function triggerNearMeLocationEngine() {
    ...
    if (!navigator.geolocation) {
        console.warn("Geolocation is not supported by your browser framework.");
        return;
    }
    if (scanRadiusActive) {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        () => { 
            console.warn("Location permission denied. Defaulting gracefully to Bhopal Center.");
            deviceCoordinates = { latitude: 23.259933, longitude: 77.412613 };
            scanRadiusActive = true;
            if (nearMeBtn) {
                nearMeBtn.classList.remove('bg-brand-gold', 'hover:bg-brand-gold-hover');
                nearMeBtn.classList.add('bg-emerald-600', 'hover:bg-emerald-700');
                nearMeBtn.innerHTML = `<i class="ph-bold ph-check text-xs"></i> Near Bhopal`;
            }
            render();
        },
...
function renderOlxChatMessages(chatData) {
    ...
    chatData.messages.forEach(m => {
        ...
        let bubbleClass = m.sender === 'user' ? "bg-slate-900 dark:bg-slate-800 text-white rounded-2xl rounded-tr-none" : "bg-white dark:bg-slate-900 text-slate-800 dark:text-white border dark:border-slate-800 rounded-2xl rounded-tl-none shadow-sm";
...
function render() {
    ...
    if(filtered.length === 0) {
        grid.innerHTML = `<div class="col-span-full py-12 text-center space-y-1.5">
            <i class="ph ph-database text-3xl text-slate-400 dark:text-white"></i>
            <p class="text-[11px] font-semibold text-slate-400 dark:text-white">Zero active asset matches found in database stream.</p>
        </div>`;
        ...
        div.innerHTML = `
            ...
            <div class="p-3 space-y-1.5">
                <div class="flex justify-between items-start gap-1">
                    ...
                    <button onclick="toggleBookmarkClientState('${x.id}', this)" class="text-lg ${x.saved ? 'text-amber-500 ph-fill' : 'text-slate-400 dark:text-white ph'} ph-bookmark"></button>
                </div>
                <p class="text-[9px] text-amber-700 dark:text-white font-extrabold bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-900/50 inline-block px-1.5 py-0.5 rounded">${x.status || 'Ready to Move'}</p>
                <p class="text-[10px] text-slate-600 dark:text-white font-semibold flex items-center gap-0.5 truncate mt-0.5"><i class="ph-fill ph-map-pin text-slate-400"></i> ${x.locality}, ${x.city}</p>
...
function renderPaginationControls(totalPages) {
    ...
    prevBtn.className = "px-2.5 py-1.5 rounded-lg border dark:border-slate-800 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-700 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-800 transition-all";
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.className = `px-2.5 py-1.5 rounded-lg border dark:border-slate-800 dark:border-slate-700 text-[10px] font-bold transition-all ${currentPropertiesPage === i ? 'bg-[#0b1329] dark:bg-slate-800 text-brand-gold border-slate-900' : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-800'}`;
        btn.innerText = i;
    ...
    nextBtn.className = "px-2.5 py-1.5 rounded-lg border dark:border-slate-800 text-[10px] font-bold bg-white dark:bg-slate-900 text-slate-700 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-800 transition-all";
...
function renderB2bServicesHub() {
    ...
    if(filteredServices.length === 0) {
        grid.innerHTML = `<div class="p-6 text-center text-slate-400 dark:text-white bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 col-span-full font-semibold text-xs animate-slide-up">No active professional partners registered in this area yet.</div>`;
        return;
    }
        ...
        div.innerHTML = `
            <div class="space-y-0.5">
                <div class="flex items-center gap-1.5 flex-wrap">
                    <h3 class="font-extrabold text-xs text-slate-900 dark:text-white uppercase tracking-tight">${x.title}</h3>
                    <span class="bg-[#0b1329] dark:bg-slate-800 text-brand-gold border border-slate-800 dark:border-slate-700 text-[8px] font-bold px-1.5 py-0.5 rounded uppercase flex items-center gap-0.5"><i class="ph-fill ph-seal-check"></i> Expert</span>
                </div>
                <p class="text-[10px] text-slate-600 dark:text-white font-semibold"><span class="text-slate-900 dark:text-white font-bold">Speciality:</span> ${x.specialty}</p>
                <div class="flex items-center gap-1.5 pt-0.5 text-[9px] font-medium">
                    ...
                    <span class="text-slate-500 dark:text-white">| ${x.city}, ${x.state}</span>
                </div>
            </div>
...
function renderMyChatsHistoryList() {
    ...
    if (activeChatsStorage.length === 0) {
        htmlContent = `<p class="text-xs text-slate-500 dark:text-white py-4 text-center font-semibold">No active conversations found yet.</p>`;
    } else {
        activeChatsStorage.forEach(chat => {
            htmlContent += `
                <div onclick="openOlxChatSimulation('${chat.propertyId}')" class="p-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-[#1e2d54] border dark:border-slate-700 rounded-xl cursor-pointer transition-all flex justify-between items-center animate-slide-up">
                    <div class="space-y-0.5">
                        <p class="text-xs font-bold text-slate-900 dark:text-white uppercase flex items-center gap-1"><i class="ph-fill ph-user-circle text-brand-gold text-base"></i> ${chat.sellerName}</p>
                        <p class="text-[9px] text-slate-500 dark:text-slate-300 font-semibold"><span class="text-slate-400">Unit:</span> ${chat.propertyTitle}</p>
                    </div>
                    <i class="ph-bold ph-caret-right text-slate-400 dark:text-white text-xs"></i>
                </div>
            `;
        });
    }
...
function renderSavedBookmarksDashboard() {
    ...
    saved.forEach(x => {
        ...
        div.innerHTML = `<div><h4 class="font-bold text-[11px] text-slate-900 dark:text-white uppercase">${x.type}</h4><span class="text-slate-900 dark:text-white font-extrabold text-[11px] block mt-0.5">₹${x.price.toLocaleString('en-IN')}</span></div><i class="ph-fill ph-caret-right text-slate-400 dark:text-white text-xs"></i>`;
        grid.appendChild(div);
    });
}
...
function renderMyPropertiesManagementWorkspace() {
    ...
    container.innerHTML = "";
    if (!sessionUser) {
        container.innerHTML = `<p class="text-xs font-semibold text-slate-500 dark:text-white py-2 text-center">Please log in to manage your listed assets.</p>`;
        return;
    }
    let myProperties = database.filter(x => String(x.phone) === String(sessionUser.phone));
    if (myProperties.length === 0) {
        container.innerHTML = `<p class="text-xs font-semibold text-slate-500 dark:text-white py-2 text-center">You have not posted any properties yet.</p>`;
        return;
    }
    myProperties.forEach(x => {
        ...
        div.innerHTML = `
            <div class="space-y-0.5">
                ...
                <p class="text-[10px] text-slate-500 dark:text-white font-medium"><i class="ph-fill ph-map-pin"></i> ${x.locality}, ${x.city}</p>
            </div>
            <div class="flex gap-2 w-full md:w-auto">
                <button onclick="deletePropertyListing('${x.id}')" class="bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-400 text-[10px] font-bold uppercase px-3.5 py-1.5 rounded-lg border border-red-200 dark:border-red-900/40 w-full md:w-auto">Delete</button>
            </div>
        `;
        container.appendChild(div);
    });
...
function updateRecentlyViewedTray() {
    ...
    if (recentItems.length === 0) {
        container.innerHTML = `<p class="text-xs text-slate-500 dark:text-white col-span-full py-4 text-center font-semibold animate-slide-up">No recently viewed listings.</p>`;
        return;
    }
    recentItems.forEach(x => {
        let div = document.createElement('div');
        div.className = "bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 p-3 rounded-xl flex justify-between items-center cursor-pointer hover:bg-slate-101 dark:hover:bg-[#1e2d54] transition-all";
        div.onclick = () => openPropertyDetailsModal(x.id);
        div.innerHTML = `<div><h4 class="font-bold text-[11px] text-slate-900 dark:text-white uppercase">${x.type}</h4><span class="text-slate-900 dark:text-white font-extrabold text-[11px] block mt-0.5">₹${x.price.toLocaleString('en-IN')}</span></div><i class="ph-fill ph-caret-right text-slate-400 dark:text-white text-xs"></i>`;
        container.appendChild(div);
    });
...
function moveStep(st) {
    ...
    stages.forEach(s => {
        ...
        if (ind) {
            if (parseInt(s) <= st) {
                ind.className = "w-3.5 h-1 bg-[#0b1329] dark:bg-white rounded-full transition-all";
            } else {
...
// Integrated signup check & secure flow state control
...
function triggerOtpRequestSequence() {
    ...
    if (otpContainer.classList.contains('hidden')) {
        ...
        // Fetching profiles database to check if user is a new signup or returning
        let existingUserDatabase = JSON.parse(localStorage.getItem('mamadealing_user_profiles')) || {};
...
function submitFirstTimeSignupDetails() {
    ...
    if (!nameVal || !emailVal || !emailVal.includes('@')) {
        alert("⚠️ Please enter a valid name and email address.");
        return;
    }
    ...
    localStorage.setItem('mamadealing_user_profiles', JSON.stringify(existingUserDatabase));

    sessionUser = {
        phone: currentSimulatedPhoneNumber,
        role: authSelectedRole,
        firmName: nameVal,
        email: emailVal
    };

    alert(`🎉 Account successfully created! Welcome, ${sessionUser.firmName}`);
    closeFeature('loginModal');
    finishAuthNodeSetup();

    // Secure database storage dispatch skip node
    fetch(`${BACKEND_BASE_URL}/api/users`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone: currentSimulatedPhoneNumber, name: nameVal, email: emailVal })
    }).catch(() => console.warn("Cloud DB sync bypassed, stored locally."));
}

function triggerGoogleLoginSequence() {
    // Premium Simulated Google Sign-In Sequence
    let simulatedName = "Gautam Sharma";
    let simulatedEmail = "gautam.sharma@gmail.com";
    let simulatedPhone = "9827011999";

    sessionUser = {
        phone: simulatedPhone,
        role: "Owner",
        firmName: simulatedName,
        email: simulatedEmail
    };

    alert(`🌐 Connected with Google Account: Welcome, ${sessionUser.firmName}!`);
    closeFeature('loginModal');
    finishAuthNodeSetup();
}
...
function logoutAgent() {
    ...
    let authComp = document.getElementById('authDynamicComponent');
    if (authComp) {
        authComp.innerHTML = `<button onclick="openFeature('loginModal')" class="bg-white hover:bg-slate-100 text-[#0b1329] dark:bg-white dark:text-slate-950 px-3.5 h-9 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center border border-slate-200 dark:border-transparent shadow-sm">Login</button>`;
    }
    renderProfileSectionWorkspace();
    switchTab('properties');
}
...
function toggleBookmarkClientState(id, el) {
    ...
    if (icon) {
        ...
        if (item.saved) {
            ...
            el.className = "text-lg text-slate-400 dark:text-white ph ph-bookmark";
        }
    }
...
function updateDetailSaveBtnVisual() {
    ...
    if (targetDetailItemInstance.saved) {
        ...
        saveIcon.className = "ph ph-bookmark text-slate-700 dark:text-white text-sm";
        saveText.innerText = "Save";
        saveBtn.className = "bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-white px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 transition-all";
    }
}
