...
function startOtpCountdownTimer() {
    ...
    const primaryBtn = document.getElementById('authPrimaryActionSubmitBtn');
    if (!timerText || !countdownSpan || !primaryBtn) return;
    ...
    otpCountdownInterval = setInterval(() => {
        ...
        if (secondsLeft <= 0) {
            ...
            primaryBtn.innerText = "Resend OTP";
        }
    }, 1000);
}
...
async function triggerOtpRequestSequence() {
    ...
    if (otpContainer.classList.contains('hidden')) {
        ...
        if (typeof window.initSendOTP === 'function' && typeof window.sendOtp !== 'function' && typeof window.sendOTP !== 'function') {
            try {
                window.initSendOTP(window.configuration);
            } catch (e) {
                console.warn("Manual initialization fallback:", e);
            }
        }
        ...
        if (sendOtpFunc && typeof sendOtpFunc === 'function') {
            try {
                ...
                isDispatched = true;
                alert("📱 Verification code sent via MSG91.");
        ...
        if (!isDispatched) {
            alert("❌ MSG91 Script Loading: Please wait 2 seconds and verify if localhost/domain is allowed in MSG91 Widget Settings.");
            return;
        }
        
        otpContainer.classList.remove('hidden');
        primaryBtn.innerText = "Verify OTP";
    } else {
        ...
        if (verifyOtpFunc && typeof verifyOtpFunc === 'function') {
            try {
                ...
                console.error("MSG91 verifyOTP failure context:", err);
            }
        }

        if (!isVerified) {
            alert("❌ Verification failed. Please re-enter the correct code sent to your phone.");
        }
    }
}
...
function renderReelsList(filterQuery = "") {
    ...
    if(reelsData.length === 0) {
        container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-center p-6 space-y-2"><i class="ph ph-video-camera-slash text-4xl text-slate-550 dark:text-white font-bold"></i><p class="text-xs font-semibold text-slate-404 dark:text-white">No matching reels found.</p></div>`;
        return;
    }
...
function calculateHaversineDistance(lat1, lon1, lat2, lon2) {
    ...
    return R * (2 * Math.atan2(window.Math.sqrt(a), window.Math.sqrt(1-a))); 
}
        ...
        div.innerHTML = `
            <div class="space-y-0.5 font-bold">
                ...
                <div class="flex items-center gap-1.5 pt-0.5 text-[9px] font-bold">
                    ...
                    <span class="text-slate-550 dark:text-slate-101 font-medium">| ${x.city}, ${state}</span>
                </div>
            </div>
...
function renderMyPropertiesManagementWorkspace() {
    ...
    container.innerHTML = "";
    if (!sessionUser) {
        container.innerHTML = `<p class="text-xs font-semibold text-slate-555 py-2 text-center">Please log in to manage your assets.</p>`;
        return;
    }
    ...
    if (authComp && sessionUser) {
        authComp.innerHTML = `
            <div class="flex items-center gap-2">
                <div class="bg-brand-gold text-slate-955 font-extrabold w-8 h-8 rounded-full flex items-center justify-center text-xs truncate max-w-[32px]">${sessionUser.firmName.slice(0,1).toUpperCase()}</div>
                <div class="text-[10px] leading-tight text-right hidden lg:block">
                    <p class="font-extrabold text-slate-800 dark:text-white">${sessionUser.firmName}</p>
...
async function triggerOtpRequestSequence() {
    ...
    if (otpContainer.classList.contains('hidden')) {
        ...
        // Manual initialization backup safeguard if window.sendOTP hasn't registered
        if (typeof window.initSendOTP === 'function' && typeof window.sendOTP !== 'function' && typeof window.sendOtp !== 'function') {
            try {
                window.initSendOTP(window.configuration);
            } catch (e) {
