...
const GEOGRAPHY_MATRIX = {
    ...
    "Odisha": ["Angul", "Balangir", "Balasore", "Bargarh", "Bhadrak", "Boudh", "Cuttack", "Deogarh", "Dhenkanal", "Gajapati", "Ganjam", "Jagatsinghpur", "Jajpur", "Jharsuguda", "Kalahandi", "Kandhamal", "Kendrapara", "Kendujhar", "Khordha", "Karoput", "Malkangiri", "Mayurbanj", "Nabarangpur", "Nayagarh", "Nuapada", "Puri", "Rayagada", "Sambalpur", "Subarnapur", "Sundargarh"],
...
// GEMINI LIVE CHAT CONFIGURATION (नई Key यहाँ जोड़ दी गई है)
const GEMINI_API_KEY = "AIzaSyAU0hGDI2XR4srKoCFpAbnRstjX7wngt0w";
...
function boostPropertyAd(id) {
    ...
    if (userSubscribedPlan === "Free") {
        ...
        item.membershipTier = userSubscribedPlan; 
...
// 🔥 LIVE GEMINI API POWERED CHATBOT ENGINE (STABLE v1 ENDPOINT INTEGRATED)
async function sendAIAssistantMessage() {
    ...
    // 1. यूजर के मैसेज को UI स्ट्रीम में दिखाएं
    appendAIChatMessage(userText, 'user');
    inputEl.value = "";

    // 2. लोडिंग इंडिकेटर सेट करें
    ...
    if (streamContainer) {
        ...
        loaderDiv.innerHTML = `<div class="bg-slate-100 text-slate-700 text-xs px-3 py-2.5 rounded-2xl max-w-[80%] font-semibold"><i class="ph-bold ph-circle-notch animate-spin mr-1"></i> AI Assistant जानकारी तैयार कर रहा है...</div>`;
        streamContainer.appendChild(loaderDiv);
        streamContainer.scrollTop = streamContainer.scrollHeight;
    }

    try {
        // 3. डेटाबेस लिस्टिंग्स को टेक्स्ट फॉर्मेट में कलेक्ट करें
        let listingsText = "";
        if (Array.isArray(database) && database.length > 0) {
            listingsText = database.slice(0, 15).map(x => 
                `- ${x.bhk || ''} ${x.type || ''} in ${x.locality || ''}, ${x.city || ''} (Price: ₹${x.price ? x.price.toLocaleString('en-IN') : 'N/A'}, Status: ${x.status || ''})`
            ).join("\n");
        } else {
            listingsText = "No active listings found in database currently.";
        }

        // सिस्टम प्रॉम्ट सेट करें
        let systemPrompt = `You are "MamaDealing AI", a friendly, local real estate assistant for Bhopal, Madhya Pradesh.
Your tone must be polite, helpful, professional, and in a natural blend of Hindi and English (Hinglish).

Here is a list of active properties currently live on our platform:
${listingsText}

Guidelines:
1. If the user asks about property rates in specific areas of Bhopal (like Neelbad, Kolar Road, Ayodhya Bypass, Awadhpuri, Katara Hills, etc.), explain the market status, approximate rates (plots per sqft, flats average cost), and neighborhood details.
2. If we have corresponding matches in the listings context above, highlight them so the user knows they are available live.
3. Offer guidance on home loans, Registry/Stamp duty processes, or listing properties.
4. Format output cleanly using bullet points and bold text where relevant. Keep it concise.`;

        // 4. Gemini API को कॉल करें (स्थिर v1 एंडपॉइंट के साथ)
        ...
        // यदि रिस्पॉन्स कोड 200 नहीं है, तो एरर को कंसोल में देखें
        if (!response.ok) {
            let errorDetails = await response.json();
            console.error("Gemini API Error Payload:", errorDetails);
        ...
        // 5. जवाब को वैलिडेट कर के UI पर रेंडर करें
        if (data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts[0]) {
            let aiReply = data.candidates[0].content.parts[0].text;
            
            // बेसिक मार्कडाउन फॉर्मेटिंग को HTML टैग्स में बदलें
            let formattedReply = aiReply
                .replace(/\*\*(.*?)\*\"/g, '<strong>$1</strong>')
                .replace(/\*(.*?)\*/g, '<em>$1</em>')
                .replace(/\n/g, '<br>');

            appendAIChatMessage(formattedReply, 'assistant');
        } else {
            console.warn("Unexpected API Response format:", data);
            appendAIChatMessage("क्षमा करें, मैं इस समय जानकारी नहीं जुटा पाया। कृपया एक बार फिर प्रयास करें।", 'assistant');
        }
        ...
        let loader = document.getElementById(loaderId);
        if (loader) loader.remove();
        appendAIChatMessage("कनेक्शन स्थापित नहीं हो सका। कृपया अपनी इंटरनेट कनेक्टिविटी और API Key की वैधता जांचें।", 'assistant');
    }
}
...
function appendAIChatMessage(text, role) {
    ...
    let bubbleClass = role === 'user' 
        ? "bg-blue-600 text-white text-xs px-3.5 py-2.5 rounded-2xl rounded-tr-none max-w-[80%] font-semibold shadow-sm" 
        : "bg-slate-100 text-slate-800 text-xs px-3.5 py-2.5 rounded-2xl rounded-tl-none max-w-[80%] font-semibold shadow-sm leading-relaxed";
...
async function fetchLivePropertiesFromDatabase() {
    try {
        console.log("🛰 *Pinged Live MongoDB Data Center Core Sequence...");
