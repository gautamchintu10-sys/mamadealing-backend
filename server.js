...
// 4. DELETE Ad Listing (सेलर विज्ञापन डिलीट करने के लिए)
app.delete('/api/properties/:id', async (req, res) => {
    try {
        ...
        if (!deletedProperty) {
            return res.status(404).json({ message: "❌ Property not found in cloud registry database." });
        }
        res.status(200).json({ message: "🎉 Property has been deleted successfully!" });
    } catch (error) {
        res.status(500).json({ message: "❌ Server error during database deletion.", error: error.message });
    }
});

// 5. POST Report Scam & Auto-Delete Logic
